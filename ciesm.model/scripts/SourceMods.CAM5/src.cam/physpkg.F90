...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    use prescribed_volc_rad,only: prescribed_volc_rad_init !wmq
    ...
    !++wy
    !use spatial_avg_tquv,   only: spatial_avg_tquv_init
    !use zm_conv,            only: zm_stc
    !--wy
    ...
    call prescribed_volc_rad_init() !wmq
...
subroutine phys_run1(phys_state, ztodt, phys_tend, pbuf2d,  cam_in, cam_out)
...
#endif
    !++wy
    !use spatial_avg_tquv, only: spatial_avg_tquv_set
    ...
    if ( adiabatic .or. ideal_phys )then
       ...
       !++wy
       !if (zm_stc) then
       !    call spatial_avg_tquv_set(phys_state)
       !end if
       !--wy
...
!>songxl 2014-11-20----
    ...
    snow_dp(:ncol) = snow_dp(:ncol) + rice(:ncol)             ! songxl 2015-09-09
...
!-----------------------------------------------------------------------------------
  ...
  use prescribed_volc_rad, only: prescribed_volc_rad_adv !wmq
  ...
  call prescribed_volc_rad_adv(phys_state, pbuf2d)

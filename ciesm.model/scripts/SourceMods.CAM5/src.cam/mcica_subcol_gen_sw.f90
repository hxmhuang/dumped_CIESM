...
! -----------------------------------ZWJ---------------------------------------
      subroutine mcica_subcol_sw(lchnk, ncol, nlay, icld, permuteseed, play, &
                       cldfrac, ciwp, clwp, rei, rel, tauc, ssac, asmc, fsfc, &
                       cldfmcl, ciwpmcl, clwpmcl, reicmcl, relqmcl, &
                       taucmcl, ssacmcl, asmcmcl, fsfcmcl, omega2, omega3, omega2mcl, omega3mcl)
! -----------------------------------ZWJ---------------------------------------
! -----------------------------------------------------------------------------
! ----- Input -----
! Control
...
! -------------------------------------------------------------------------------------
! --------------------------------------------ZWJ--------------------------------------
      real(kind=r8), intent(in) :: omega2(:,:,:)
      real(kind=r8), intent(in) :: omega3(:,:,:)
      real(kind=r8), intent(out) :: omega2mcl(:,:,:)
      real(kind=r8), intent(out) :: omega3mcl(:,:,:)
! --------------------------------------------ZWJ--------------------------------------
! -------------------------------------------------------------------------------------
! ----- Output -----
...
! ------------------------------------------------ZWJ---------------------------------------------------
      call generate_stochastic_clouds_sw (ncol, nlay, nsubcsw, icld, pmid, cldfrac, clwp, ciwp, tauc, &
                               ssac, asmc, fsfc, cldfmcl, clwpmcl, ciwpmcl, taucmcl, ssacmcl, asmcmcl, fsfcmcl, permuteseed, &
                               omega2, omega3, omega2mcl, omega3mcl)
! ------------------------------------------------ZWJ---------------------------------------------------
! ------------------------------------------------------------------------------------------------------
...
! -----------------ZWJ-------------
      subroutine generate_stochastic_clouds_sw(ncol, nlay, nsubcol, icld, pmid, cld, clwp, ciwp, tauc, & 
                                   ssac, asmc, fsfc, cld_stoch, clwp_stoch, ciwp_stoch, tauc_stoch, &
                                   ssac_stoch, asmc_stoch, fsfc_stoch, changeSeed, omega2, omega3, omega2_stoch, omega3_stoch)
! -----------------ZWJ-------------
! ---------------------------------
!-------------------------------------------------------------------------------------------------
...
! ---------------------------------ZWJ----------------------------------------------
      real(kind=r8), intent(in) :: omega2(:,:,:)
      real(kind=r8), intent(in) :: omega3(:,:,:)
      real(kind=r8), intent(out) :: omega2_stoch(:,:,:)
      real(kind=r8), intent(out) :: omega3_stoch(:,:,:)
! ---------------------------------ZWJ----------------------------------------------
! ----------------------------------------------------------------------------------
...
! --------------------------------------------------------------------
! -------------------------------ZWJ----------------------------------
                  omega2_stoch(isubcol,i,ilev) = omega2(n,i,ilev)
                  omega3_stoch(isubcol,i,ilev) = omega3(n,i,ilev)
! -------------------------------ZWJ----------------------------------
! --------------------------------------------------------------------
               else
...
! ------------------------------------------------------
! --------------------------ZWJ-------------------------
                  omega2_stoch(isubcol,i,ilev) = 0._r8
                  omega3_stoch(isubcol,i,ilev) = 0._r8
! --------------------------ZWJ-------------------------
! ------------------------------------------------------
               endif
            enddo

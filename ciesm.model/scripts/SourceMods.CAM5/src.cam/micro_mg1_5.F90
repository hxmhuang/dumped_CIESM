...
!-----wmq-----
use cam_control_mod,  only : cmip6_forcing
!-------------
...
!-----wmq-----
  ...
  real(r8) :: lamc_rad(mgncol,nlev)   ! slope for radiation
  real(r8) :: pgam_rad(mgncol,nlev)   ! spectral width parameter for radiation
!-------------
...
!-----------wmq----------
           ...
           call size_dist_param_liq_rad(eta(i), dumc(i,k), dumnc(i,k), cdnl, rho(i,k), .false., &
                pgam_rad(i,k), lamc_rad(i,k))

           !assign output fields for shape here
           if (cmip6_forcing) then
               lamcrad(i,k)=lamc_rad(i,k)
               pgamrad(i,k)=pgam_rad(i,k)
           else
...
!---------wmq-----------
! get cloud droplet size distribution parameters
elemental subroutine size_dist_param_liq_rad(eta, qcic, ncic, cdnl, rho, nadjflag, pgam, lamc)
  ...
  if (qcic > qsmall) then
     ...
     pgam=0.0005714_r8*(ncic*eta/1.e6_r8*rho)+0.2714_r8
     pgam=1._r8/(pgam**2)-1._r8
     pgam=max(pgam,2._r8)
     pgam=min(pgam,15._r8)
     ...
     lamc = (pi/6._r8*rhow*ncic*eta*dumgam2/ &
          (qcic*dumgam1))**(1._r8/3._r8)
     ...
     ! pgam not calculated in this case, so set it to a value likely to cause an
     ! error
     ! if it is accidentally used
     ! (gamma function undefined for negative integers)
     pgam = -100._r8
...
end subroutine size_dist_param_liq_rad
!-----------------------
...
!qy         2._r8*1350._r8 * qcic**2.47_r8 * (ncic/1.e6_r8*rho)**(-1.79_r8)

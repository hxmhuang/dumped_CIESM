...
use cam_control_mod,  only: N_CMIP, cmip6_forcing  !wmq
use radconstants,     only: get_sw_spectral_boundaries !wmq
use parrrtm,          only: nbndlw !wmq
use parrrsw,          only: nbndsw !wmq
use tropopause,   only : tropopause_find, TROP_ALG_TWMO, TROP_ALG_CLIMATE !wmq
use prescribed_volc_rad, only: prescribed_volc_rad_set !wmq
use cam_logfile,  only : iulog !wmq
...
!wmq
    real(r8) :: pi_tau    (pcols,0:pver,nbndsw)
    real(r8) :: pi_tau_w  (pcols,0:pver,nbndsw)
    real(r8) :: pi_tau_w_g(pcols,0:pver,nbndsw)
    real(r8) :: pi_tau_w_f(pcols,0:pver,nbndsw)
    real(r8) :: pi_lw_abs (pcols,pver,nbndlw)
    real(r8) :: frcing_sw(pcols,pver,nbndsw,3)
    real(r8) :: frcing_lw(pcols,pver,nbndlw,3)
...
subroutine aer_rad_props_init()
   ...
   use cam_history_support,   only: register_hist_mdim !wmq
   ...
   call addfld ('AEROD_trop ', '1', 1, 'A', &
      'Total Aerosol Optical Depth in visible band in tropsphere', phys_decomp, flag_xyfill=.true.)

   call addfld ('MAC_aod ', '1', 1, 'A', &
      'MACv2-SP Aerosol Optical Depth in visible band', phys_decomp, flag_xyfill=.true.)

   call addfld ('zdelt', '1', pver, 'A', &
      'height between levels', phys_decomp, flag_xyfill=.true.)
   ...
   if (history_amwg ) then 
      ...
      call add_default ('MAC_aod', 1, ' ')  
 
   if ( history_aero_optics ) then 
      call add_default ('AEROD_v', 1, ' ')
      if (cmip6_forcing) then
          call add_default ('AEROD_trop', 1, ' ')
      end if
      do i = 1, numaerosols
         odv_names(i) = 'ODV_'//trim(aernames(i))
...
subroutine aer_rad_props_sw(list_idx, state, pbuf,  nnite, idxnite, &
   ...
   use physics_buffer, only : physics_buffer_desc
   use MAC_SP_forcing,   only : MAC_SP_set_rad !wmq
   ...
   real(r8), intent(out) :: tau_w_f(pcols,0:pver,nswbands) ! aerosol forward scattered fraction * tau * w
    ...
    integer ns
    real(r8) ::  frcing(pcols,pver,N_CMIP)
    real(r8) ::  MAC_aod(pcols)
    real(r8) ::  cmip_aod(pcols,pver,nbndsw)
    real(r8) ::  cmip_ssa(pcols,pver,nbndsw)
    real(r8) ::  cmip_asy(pcols,pver,nbndsw)
    real(r8) ::  cmip_f(pcols,pver,nbndsw)
    real(r8) ::  wavmin(nbndsw)
    real(r8) ::  wavmax(nbndsw)
    real(r8) ::  wavmid(nbndsw) ! Spectral interval middle wavelength

   real(r8) :: SAD_ext(pcols,pver,nbndsw)
   real(r8) :: SAD_ssa(pcols,pver,nbndsw)
   real(r8) :: SAD_asm(pcols,pver,nbndsw)
...
!---wmq:for MACv2-SP and CMIP6_SAD_V3
   do k = 1,pver
      do i = 1,ncol
         zdelt(i,k) = ( state%zi(i,k)-state%zi(i,k+1) ) /1000._r8
      end do
   end do

if (cmip6_forcing) then
   frcing(:,:,:) = 0._r8
   call MAC_SP_set_rad( lchnk, frcing, ncol )  ! get MACv2 SP data

   call get_sw_spectral_boundaries(wavmin,wavmax,'nanometers')
   do ns = 1, nswbands
      wavmid(ns) = 0.5_r8*(wavmin(ns) + wavmax(ns))
   end do

   do ns = 1, nswbands
      cmip_aod(:ncol,:,ns)=frcing(:ncol,:,1)*exp(-2._r8*log(wavmid(ns)/550._r8))
      if (wavmid(ns) .gt. 700._r8) then
          cmip_ssa(:ncol,:,ns)= frcing(:ncol,:,2)* ( frcing(:ncol,:,2)+(1._r8-frcing(:ncol,:,2)) &
                                *(wavmid(ns)/700._r8)**3._r8 )**(-1._r8)
          cmip_asy(:ncol,:,ns)=frcing(:ncol,:,3)* ( wavmid(ns)/700._r8)**(-1._r8/2._r8)
      else
          cmip_ssa(:ncol,:,ns) = frcing(:ncol,:,2) * 1._r8
          cmip_asy(:ncol,:,ns) = frcing(:ncol,:,3) * 1._r8
      end if
   end do
   cmip_f(:ncol,:,:)=cmip_asy(:ncol,:,:)**2

!----TEST for MACv2-SP AOD----
   MAC_aod = 0._r8
   MAC_aod(1:ncol) = sum(cmip_aod(1:ncol,:,idx_sw_diag), 2)
   ! use fillvalue to indicate night columns
   do i = 1, nnite
      MAC_aod(idxnite(i)) = fillvalue
   end do
   call outfld('MAC_aod', MAC_aod, pcols, lchnk)
   call outfld('zdelt', zdelt, pcols, lchnk)
  ! call endrun
!-----------------------------

! shortwave aerosol optical extiction (background + MACv2 SP)

   tau(:ncol,1:pver,:)     = tau(:ncol,1:pver,:) &
                           + cmip_aod(:ncol,1:pver,:)

   tau_w(:ncol,1:pver,:)   = tau_w(:ncol,1:pver,:) &
                           + cmip_ssa(:ncol,1:pver,:)* cmip_aod(:ncol,1:pver,:)

   tau_w_g(:ncol,1:pver,:) = tau_w_g(:ncol,1:pver,:) &
                           + cmip_asy(:ncol,1:pver,:)* cmip_ssa(:ncol,1:pver,:)*cmip_aod(:ncol,1:pver,:)

   tau_w_f(:ncol,1:pver,:) = tau_w_f(:ncol,1:pver,:) &
                           + cmip_f(:ncol,1:pver,:)* cmip_ssa(:ncol,1:pver,:)*cmip_aod(:ncol,1:pver,:)
   ...
   do iaerosol = 1, numaerosols
      ...
      ! get optics type
      if(trim(opticstype).eq.'volcanic' .or. trim(opticstype).eq.'volcanic_radius') then
           opticstype=trim('CMIP6_SAD_V3')
      end if
      ...
      case('CMIP6_SAD_V3')
         ...
         frcing_sw(:,:,:,:)=0._r8
         frcing_lw(:,:,:,:)=0._r8
         call prescribed_volc_rad_set( lchnk, frcing_sw, frcing_lw, ncol )

       do ns = 1, nswbands
         do k = 1,pver
            do i = 1,ncol
               SAD_ext(i,k,ns)=frcing_sw(i,k,ns,1) * zdelt(i,k)
               SAD_ssa(i,k,ns)=frcing_sw(i,k,ns,2)
               SAD_asm(i,k,ns)=frcing_sw(i,k,ns,3)
            end do
          end do
       end do
   ...
   do ns = 1, nswbands
      ta (1:ncol,1:pver,ns) = SAD_ext(1:ncol,1:pver,ns)

      tw (1:ncol,1:pver,ns) = SAD_ext(1:ncol,1:pver,ns) * SAD_ssa(1:ncol,1:pver,ns)

      twg(1:ncol,1:pver,ns) = SAD_ext(1:ncol,1:pver,ns) &
                            * SAD_ssa(1:ncol,1:pver,ns) * SAD_asm(1:ncol,1:pver,ns)

      twf(1:ncol,1:pver,ns) = SAD_ext(1:ncol,1:pver,ns) &
                            * SAD_ssa(1:ncol,1:pver,ns) * SAD_asm(1:ncol,1:pver,ns) &
                            * SAD_asm(1:ncol,1:pver,ns)
   enddo
       ...
       do i = 1,ncol
          do k = 1,pver
             ...
             if ( k > tropLev(i) ) then
                 ...
                 twg(i,k,:) = 0._r8
                 twf(i,k,:) = 0._r8
             else if (k < tropLev(i) ) then
                 tau(i,k,:)     = 0._r8
                 tau_w  (i,k,:) = 0._r8
                 tau_w_g(i,k,:) = 0._r8
                 tau_w_f(i,k,:) = 0._r8
             else
                 tau(i,k,:)     = 0.5_r8 * tau(i,k,:)
                 tau_w  (i,k,:) = 0.5_r8 * tau_w  (i,k,:)
                 tau_w_g(i,k,:) = 0.5_r8 * tau_w_g(i,k,:)
                 tau_w_f(i,k,:) = 0.5_r8 * tau_w_f(i,k,:)
                 ta (i,k,:) = 0.5_r8 * ta (i,k,:)
                 tw (i,k,:) = 0.5_r8 * tw (i,k,:) 
                 twg(i,k,:) = 0.5_r8 * twg(i,k,:)
                 twf(i,k,:) = 0.5_r8 * twf(i,k,:)
             endif
           end do
        end do
         tau    (1:ncol,1:pver,:) = tau    (1:ncol,1:pver,:) + ta (1:ncol,:,:)
...
subroutine aer_rad_props_lw(list_idx, state, pbuf,  odap_aer)
   ...
   !---wmq---Output for Aerosol radiative properties----
    integer m, ns
   real(r8) :: ext_lw(pcols,pver,nbndlw)
   real(r8) :: zdelt(pcols,pver)
   !---------------------------------------------------- 
   ...
   do iaerosol = 1, numaerosols
      ...
      case('volcanic_radius')
       ...
       do ns = 1, nlwbands
         do k = 1,pver
            do i = 1,ncol
               ext_lw(i,k,ns)=frcing_lw(i,k,ns,1) * zdelt(i,k)
            end do
         end do
       end do
         ...
         do i = 1,ncol
           do k = 1,pver
             ...
             if ( k > tropLev(i) ) then
                 ...
                 ext_lw  (i,k,:) = 0.5_r8 * ext_lw  (i,k,:)
                 odap_aer(i,k,:) = 0.5_r8 * odap_aer(i,k,:)
             endif
           end do
         end do

       odap_aer(1:ncol,1:pver,:) = odap_aer(1:ncol,1:pver,:) + ext_lw(1:ncol,:,:)
...
subroutine get_sad_rad_props(ncol, mass, ext, ssa, asm, &
                                  tau, tau_w, tau_w_g, tau_w_f)

   ! Arguments
   integer,  intent(in) :: ncol
   real(r8), intent(in) :: mass(pcols, pver)
   real(r8), intent(in) :: ext(pcols, pver, nswbands)
   real(r8), intent(in) :: ssa(pcols, pver, nswbands)
   ...
   do iswband = 1, nswbands
      tau    (1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext(1:ncol,1:pver,iswband)
      tau_w  (1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext(1:ncol,1:pver,iswband) & 
                                     * ssa(1:ncol,1:pver,iswband)
      tau_w_g(1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext(1:ncol,1:pver,iswband) &
                                     * ssa(1:ncol,1:pver,iswband) * asm(1:ncol,1:pver,iswband)
      tau_w_f(1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext(1:ncol,1:pver,iswband) &
                                     * ssa(1:ncol,1:pver,iswband) * asm(1:ncol,1:pver,iswband) &
                                     * asm(1:ncol,1:pver,iswband)
   enddo

end subroutine get_sad_rad_props
...
subroutine aer_vis_diag_out(lchnk, ncol, nnite, idxnite, iaer, tau, diag_idx)
   ...
   if (iaer > 0) then
      ...
      call outfld('AEROD_trop', tmp, pcols, lchnk)
   else

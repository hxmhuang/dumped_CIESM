...
!===============================================================================
  ...
  subroutine init_uwshcu( kind, xlv_in, cp_in, xlf_in, zvir_in, r_in, g_in, ep2_in )
    ...
    ! qinyi 2017-11-30 19:52:20
    call addfld( 'qtu_bef_expl_Cu', 'kg/kg'   , pver  , 'A' , 'qtu before expelling to the environment'             , phys_decomp )
    call addfld( 'explw_Cu'       , 'kg/kg'   , pver  , 'A' , 'Expelled cloud water to env.'                        , phys_decomp )
    call addfld( 'expli_Cu'       , 'kg/kg'   , pver  , 'A' , 'Expelled ice water to env.'                          , phys_decomp )
    call addfld( 'explwi_Cu'      , 'kg/kg'   , pver  , 'A' , 'Expelled water and ice water to env.'                , phys_decomp )
    call addfld( 'explw2qtu_Cu'   , 'fraction', pver  , 'A' , 'Expelled cloud water to env./qtu'                    , phys_decomp )
    call addfld( 'expli2qtu_Cu'   , 'fraction', pver  , 'A' , 'Expelled ice water to env./qtu'                      , phys_decomp )
    call addfld( 'explwi2qtu_Cu'  , 'fraction', pver  , 'A' , 'Expelled cloud and ice water to env./qtu'            , phys_decomp )
    ! qinyi
...
! qinyi
  subroutine compute_uwshcu_inv( mix      , mkx        , iend          , ncnst     , dt       ,  & 
                                 ...
                                 explwi2qtu_inv) ! qinyi 2017-11-30 21:32:08
...
! qinyi
    ...
    ! qinyi 2017-11-30 21:32:32
    real(r8), intent(out)   :: explwi2qtu_inv(mix,mkx)
    ! qinyi
...
! qinyi
    ...
    ! qinyi 2017-11-30 21:33:10
    real(r8)                :: explwi2qtu(mix,mkx)
    ! qinyi
...
! qinyi
    call compute_uwshcu( mix  , mkx    , iend      , ncnst , dt   , &
                         ...
                         qtu  , thlu, & !qinyi 
                         explwi2qtu) ! qinyi 2017-11-30 21:33:55
    ...
    ! qinyi 2017-11-30 21:34:03
    explwi2qtu_inv(:iend,:) = explwi2qtu(:iend,:)
    ! qinyi
    enddo
...
! qinyi
  subroutine compute_uwshcu( mix      , mkx       , iend         , ncnst    , dt        , &
                             ...
                             explwi2qtu_out) ! qinyi 2017-11-30 21:35:34
...
! qinyi
    ...
    ! qinyi 2017-11-30 21:35:50
    real(r8), intent(out)   :: explwi2qtu_out(mix,mkx)
    ! qinyi
   ...
   ! 3. Variables associated with cumulus convection
    ...
    ! qinyi 2017-11-30 20:08:02
    real(r8)    qtu_bef_expl(mkx)
    real(r8)    explw(mkx)
    real(r8)    expli(mkx)
    real(r8)    explwi(mkx)
    real(r8)    explw2qtu(mkx)
    real(r8)    expli2qtu(mkx)
    real(r8)    explwi2qtu(mkx)
    ! qinyi
...
!    real(r8)  thlu_out(mix,0:mkx)                             !  Updraft thl [ K ]
    ...
    ! qinyi 2017-11-30 20:09:44
    real(r8)  qtu_bef_expl_out(mix,mkx)
    real(r8)  explw_out(mix,mkx)
    real(r8)  expli_out(mix,mkx)
    real(r8)  explwi_out(mix,mkx)
    real(r8)  explw2qtu_out(mix,mkx)
    real(r8)  expli2qtu_out(mix,mkx)
!    real(r8)  explwi2qtu_out(mix,mkx)
    ...
    ! qinyi 2017-11-30 20:11:30
    real(r8)  qtu_bef_expl_s(mkx)
    real(r8)  explw_s(mkx)
    real(r8)  expli_s(mkx)
    real(r8)  explwi_s(mkx)
    real(r8)  explw2qtu_s(mkx)
    real(r8)  expli2qtu_s(mkx)
    real(r8)  explwi2qtu_s(mkx)
    ! qinyi
...
!   parameter (rkm = 16.0_r8)        !  Determine the amount of air that is involved in buoyancy-sorting [no unit] 
    ...
    ! qinyi 2017-11-30 20:12:11
    qtu_bef_expl_out(:iend,:mkx)          = 0.0_r8
    explw_out(:iend,:mkx)                 = 0.0_r8
    expli_out(:iend,:mkx)                 = 0.0_r8
    explwi_out(:iend,:mkx)                = 0.0_r8
    explw2qtu_out(:iend,:mkx)             = 0.0_r8
    expli2qtu_out(:iend,:mkx)             = 0.0_r8
    explwi2qtu_out(:iend,:mkx)            = 0.0_r8
    ! qinyi
    ...
    do i = 1, iend                                      
      ...
      dwten(:mkx)         = 0.0_r8
      diten(:mkx)         = 0.0_r8
      ! qinyi 2017-11-30 20:14:32
      qtu_bef_expl(:mkx)  = 0.0_r8
      explw(:mkx)         = 0.0_r8
      expli(:mkx)         = 0.0_r8
      explwi(:mkx)        = 0.0_r8
      explw2qtu(:mkx)     = 0.0_r8
      expli2qtu(:mkx)     = 0.0_r8
      explwi2qtu(:mkx)    = 0.0_r8
      ! qinyi
      precip              = 0.0_r8
      snow                = 0.0_r8
      evapc(:mkx)         = 0.0_r8
...
25     continue
     ...
     !                'cinlfc'.
       ...
       if( iter .ne. 1 ) then
           ...
           if( del_CIN .gt. 0._r8 ) then
               ...
               ! qinyi 2017-11-30 20:17:10
               qtu_bef_expl_out(i,mkx:1:-1)       = qtu_bef_expl_s(:mkx)
               explw_out(i,mkx:1:-1)              = explw_s(:mkx)
               expli_out(i,mkx:1:-1)              = expli_s(:mkx)
               explwi_out(i,mkx:1:-1)             = explwi_s(:mkx)
               explw2qtu_out(i,mkx:1:-1)          = explw2qtu_s(:mkx)
               expli2qtu_out(i,mkx:1:-1)          = expli2qtu_s(:mkx)
               explwi2qtu_out(i,mkx:1:-1)         = explwi2qtu_s(:mkx)
               ! qinyi
       ...
       do iter_xc = 1, niter_xc
        ...
        ! rei(k) = ( rkm / scaleh / g / rho0j )        ! Default.
          ...
          if( (qlj + qij) .gt. criqc ) then
               ...
               dwten(k) = exql   
               diten(k) = exqi
               ! qinyi 2017-11-30 20:19:23
               qtu_bef_expl(k)      = qtu(k)+exql+exqi
               explw(k)             = exql
               expli(k)             = exqi
               explwi(k)            = explw(k)+expli(k)
               if(qlj.ne.0.0_r8)then
                 explw2qtu(k)         = explw(k)/qlj
               else
                 explw2qtu(k)         = 0.0_r8
               end if

               if(qij.ne.0.0_r8)then
                 expli2qtu(k)         = expli(k)/qij
               else
                 expli2qtu(k)         = 0.0_r8
               end if

               if((qlj+qij).ne.0.0_r8)then
                 explwi2qtu(k)        = explwi(k)/(qlj+qij)
               else
                 explwi2qtu(k)        = 0.0_r8
               end if
               ! qinyi
 ...
 45    continue
       ...
       if( (qlj + qij) .gt. criqc ) then
            ...
            ! qinyi 2017-11-30 20:26:01
            qtu_bef_expl(kpen)      = qtu_top+dwten(kpen)+diten(kpen)
            explw(kpen)             = dwten(kpen)
            expli(kpen)             = diten(kpen)
            explwi(kpen)            = explw(kpen)+expli(kpen)
            if(qlj.ne.0.0_r8)then
              explw2qtu(kpen)         = explw(kpen)/qlj
            else
              explw2qtu(kpen)         = 0.0_r8
            end if

            if(qij.ne.0.0_r8)then
              expli2qtu(kpen)         = expli(kpen)/qij
            else
              expli2qtu(kpen)         = 0.0_r8
            end if

            if((qlj+qij).ne.0.0_r8)then
              explwi2qtu(kpen)        = explwi(kpen)/(qlj+qij)
            else
              explwi2qtu(kpen)        = 0.0_r8
            end if
            ! qinyi
       ...
       ! qinyi 2017-11-30 20:27:40
       explw(kpen+1:mkx)              = 0.0_r8
       expli(kpen+1:mkx)              = 0.0_r8
       explwi(kpen+1:mkx)             = 0.0_r8
       explw2qtu(kpen+1:mkx)          = 0.0_r8
       expli2qtu(kpen+1:mkx)          = 0.0_r8
       explwi2qtu(kpen+1:mkx)         = 0.0_r8
       ! qinyi
       ...
       if( iter .ne. iter_cin ) then 
          ...
          ! qinyi 2017-11-30 20:30:04
          qtu_bef_expl_s(:mkx)  = qtu_bef_expl(:mkx)
          explw_s(:mkx)         = explw(:mkx)
          expli_s(:mkx)         = expli(:mkx)
          explwi_s(:mkx)        = explwi(:mkx)
          explw2qtu_s(:mkx)     = explw2qtu(:mkx)
          expli2qtu_s(:mkx)     = expli2qtu(:mkx)
          explwi2qtu_s(:mkx)    = explwi2qtu(:mkx)
          ! qinyi
...
!the indices are not reversed, these variables go into compute_mcshallow_inv, this is why they are called "flxprc1" and "flxsnow1". 
     ...
     ! qinyi 2017-11-30 20:31:34
     qtu_bef_expl_out(i,mkx:1:-1)         = qtu_bef_expl(:mkx)
     explw_out(i,mkx:1:-1)                = explw(:mkx)
     expli_out(i,mkx:1:-1)                = expli(:mkx)
     explwi_out(i,mkx:1:-1)               = explwi(:mkx)
     explw2qtu_out(i,mkx:1:-1)            = explw2qtu(:mkx)
     expli2qtu_out(i,mkx:1:-1)            = expli2qtu(:mkx)
     explwi2qtu_out(i,mkx:1:-1)           = explwi2qtu(:mkx)
     ! qinyi
 ...
 333 if(id_exit) then ! Exit without cumulus convection
     ...
     ! qinyi 2017-11-30 20:33:45
     qtu_bef_expl_out(i,mkx:1:-1)       = 0.0_r8 
     explw_out(i,mkx:1:-1)              = 0.0_r8 
     expli_out(i,mkx:1:-1)              = 0.0_r8 
     explwi_out(i,mkx:1:-1)             = 0.0_r8 
     explw2qtu_out(i,mkx:1:-1)          = 0.0_r8 
     expli2qtu_out(i,mkx:1:-1)          = 0.0_r8 
     explwi2qtu_out(i,mkx:1:-1)         = 0.0_r8 
     ! qinyi
     ...
     ! qinyi 2017-11-30 19:59:47
     call outfld( 'qtu_bef_expl_Cu'         , qtu_bef_expl_out,               mix,          lchnk )
     call outfld( 'explw_Cu'                , explw_out       ,               mix,          lchnk )
     call outfld( 'expli_Cu'                , expli_out       ,               mix,          lchnk )
     call outfld( 'explwi_Cu'               , explwi_out      ,               mix,          lchnk )
     call outfld( 'explw2qtu_Cu'            , explw2qtu_out   ,               mix,          lchnk )
     call outfld( 'expli2qtu_Cu'            , expli2qtu_out   ,               mix,          lchnk )
     call outfld( 'explwi2qtu_Cu'           , explwi2qtu_out  ,               mix,          lchnk )
     ! qinyi 

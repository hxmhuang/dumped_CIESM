...
!++wy tuning
   integer  ::    lambdadpcu_idx   = -1
   integer  ::    mudpcu_idx       = -1
!--wy tuning
...
!zx   
   call addfld ('RIMING', 'degree', pver, 'A', 'Riming intensity of ice'   ,phys_decomp)
   call addfld ('SWC',    'm-3',    pver, 'A', 'snow water conc'           ,phys_decomp)   ! In cloud snow water path for radiation
...
!zx
      call add_default ('RIMING   ', budget_histfile, ' ')
      call add_default ('SWC   ', budget_histfile, ' ')
...
!-----wmq-----
   use MAC_SP_forcing,   only : MAC_SP_set_cdnc
   use cam_control_mod,  only : cmip6_forcing, N_CMIP
!-------------
  ...
  !---wmq---
   real(r8) :: flux(pcols,N_CMIP)
   real(r8) :: eta(pcols)
  !---------

!zx  
   real(r8) :: riming(pcols,pver)  
   real(r8) :: swc(pcols,pver)
   swc=0_r8
   !-------------------------------------------------------------------------------
  ...
  !-------wmq-------
   if (cmip6_forcing) then
       flux(:,:) = 0._r8
       eta(:) = 0._r8
       call MAC_SP_set_cdnc( lchnk, flux, ncol )  ! get MACv2 SP data
       eta(1:ncol)= flux(1:ncol,4)
   end if
  !-----------------
   ...
   do k = top_lev, pver
      do i = 1, ncol 
         ...
         if ( ast(i,k) < 1.e-4_r8 ) then
            ...
            mu(i,k) = mudpcu(i,k)
            lammin = (mudpcu(i,k)+1._r8)/50.e-6_r8
            lammax = (mudpcu(i,k)+1._r8)/2.e-6_r8
            lambdac(i,k) = lambdadpcu(i,k)
            if (lambdac(i,k) < lammin) then
                lambdac(i,k) = lammin
            else if (lambdac(i,k) > lammax) then
                lambdac(i,k) = lammax
            end if
            !--wy tuning
            dei(i,k) = deicon
   ...
   do k = top_lev, pver
      do i = 1, ncol
         ...
         swc(i,k) = qsout(i,k) * state_loc%pdel(i,k) / gravit
      end do
   end do
...
!zx
   call outfld( 'RIMING'  , riming,     pcols, lchnk )
   call outfld( 'SWC'     , swc,        pcols, lchnk )        ! In cloud snow water path for radiation

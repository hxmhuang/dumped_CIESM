...
subroutine rad_rrtmg_sw(lchnk,ncol       ,rrtmg_levs   ,r_state      , &
                    ...
                    su       ,sd         ,                             &
                    E_cld_tau, E_cld_tau_w, E_cld_tau_w_g, E_cld_tau_w_f,  &
                    E_cld_tau_w_omega2, E_cld_tau_w_omega3, old_convert)
! --------------------------------------ZWJ----------------------------------
! ---------------------------------------------------------------------------
!-----------------------------------------------------------------------
! 
...
! --------------------------------------------------------------------------------------------
! ----------------------------------------ZWJ-------------------------------------------------
   real(r8), optional, intent(in) :: E_cld_tau_w_omega2(nbndsw, pcols, pver)
   real(r8), optional, intent(in) :: E_cld_tau_w_omega3(nbndsw, pcols, pver)
! ----------------------------------------ZWJ-------------------------------------------------
! --------------------------------------------------------------------------------------------
   ! Output arguments
...
! -----------------------------------------------------------------------------------------------------------
! -----------------------------------------------ZWJ---------------------------------------------------------
   real(r8) :: omega2_sw(nbndsw, pcols, rrtmg_levs-1)
   real(r8) :: omega3_sw(nbndsw, pcols, rrtmg_levs-1)
   real(r8) :: omega2_stosw(nsubcsw, pcols, rrtmg_levs-1)
   real(r8) :: omega3_stosw(nsubcsw, pcols, rrtmg_levs-1)
! -----------------------------------------------ZWJ---------------------------------------------------------
! -----------------------------------------------------------------------------------------------------------
...
! ----------------------------------------------------------------------------------------------
! -----------------------------------------------ZWJ--------------------------------------------
              omega2_sw(ns,i,k)=E_cld_tau_w_omega2(ns,IdxDay(i),kk)/E_cld_tau_w(ns,IdxDay(i),kk)
              omega3_sw(ns,i,k)=E_cld_tau_w_omega3(ns,IdxDay(i),kk)/E_cld_tau_w(ns,IdxDay(i),kk)
! -----------------------------------------------ZWJ--------------------------------------------
! ----------------------------------------------------------------------------------------------
           else
...
! ----------------------------------------------------------------------------------------------
! -----------------------------------------------ZWJ--------------------------------------------
              omega2_sw(ns,i,k) = 0._r8
              omega3_sw(ns,i,k) = 0._r8
! -----------------------------------------------ZWJ--------------------------------------------
! ----------------------------------------------------------------------------------------------
           endif
...
! ----------------------------------------------------------------------------------------------
! -----------------------------------------------ZWJ--------------------------------------------
              omega2_sw(ns,i,k)=E_cld_tau_w_omega2(ns,IdxDay(i),kk)/max(E_cld_tau_w(ns,IdxDay(i),kk), 1.e-80_r8)
              omega3_sw(ns,i,k)=E_cld_tau_w_omega3(ns,IdxDay(i),kk)/max(E_cld_tau_w(ns,IdxDay(i),kk), 1.e-80_r8)
! -----------------------------------------------ZWJ--------------------------------------------
! ----------------------------------------------------------------------------------------------
           else
              fsfc_sw(ns,i,k) = 0._r8
...
! ---------------------------------------------ZWJ---------------------------------------------
   call mcica_subcol_sw(lchnk, Nday, rrtmg_levs-1, icld, permuteseed, pmid, &
      cld, cicewp, cliqwp, rei, rel, tauc_sw, ssac_sw, asmc_sw, fsfc_sw, &
      cld_stosw, cicewp_stosw, cliqwp_stosw, rei_stosw, rel_stosw, &
      tauc_stosw, ssac_stosw, asmc_stosw, fsfc_stosw, omega2_sw, omega3_sw, omega2_stosw, omega3_stosw)
! ---------------------------------------------ZWJ---------------------------------------------
! ---------------------------------------------------------------------------------------------
...
! -----------------------------------------ZWJ---------------------------------------------
   call rrtmg_sw(lchnk, Nday, rrtmg_levs, icld,         &
                 ...
                 tau_aer_sw, ssa_aer_sw, asm_aer_sw, &
                 swuflx, swdflx, swhr, swuflxc, swdflxc, swhrc, &
                 dirdnuv, dirdnir, difdnuv, difdnir, ninflx, ninflxc, swuflxs, swdflxs, omega2_stosw, omega3_stosw)
! -----------------------------------------ZWJ---------------------------------------------
! -----------------------------------------------------------------------------------------

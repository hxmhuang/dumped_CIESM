...
! ------------------------------------------ZWJ--------------------------------------
      subroutine spcvmc_sw &
            (lchnk, iplon, nlayers, istart, iend, icpr, idelm, iout, &
             ...
             pbbfd, pbbfu, pbbcd, pbbcu, puvfd, puvcd, pnifd, pnicd, pnifu, pnicu, &
             pbbfddir, pbbcddir, puvfddir, puvcddir, pnifddir, pnicddir, &
             pbbfsu, pbbfsd, pomega2mc, pomega3mc, pfsfcmc)
! ------------------------------------------ZWJ--------------------------------------
! -----------------------------------------------------------------------------------
!
...
! ---------------------------------------------------------------------
! ---------------------------------ZWJ---------------------------------
      real(kind=r8), intent(in) :: pomega2mc(:,:)
      real(kind=r8), intent(in) :: pomega3mc(:,:)
      real(kind=r8), intent(in) :: pfsfcmc(:,:)
! ---------------------------------ZWJ---------------------------------
! ---------------------------------------------------------------------                                                                 !   Dimensions: (nlayers)
...
! ------------------------------------------------------------
! ------------------------------ZWJ---------------------------
      real(kind=r8) :: zrdfc(nlayers,2,2),ztdfc(nlayers,2,2)
      real(kind=r8) :: zrdrc(nlayers,2),ztdrc(nlayers,2)
      real(kind=r8) :: zrdfo(nlayers,2,2),ztdfo(nlayers,2,2)
      real(kind=r8) :: zrdro(nlayers,2),ztdro(nlayers,2)
      real(kind=r8) :: zrdr(nlayers,2),zrdf(nlayers,2,2)
      real(kind=r8) :: ztdr(nlayers,2),ztdf(nlayers,2,2)
      integer :: kmodts, ic, jc
      real(kind=r8) :: omegac1(nlayers),omegac2(nlayers),omegac3(nlayers)
      real(kind=r8) :: omegao1(nlayers),omegao2(nlayers),omegao3(nlayers)
! ------------------------------ZWJ---------------------------
! ------------------------------------------------------------
! Inactive arrays
...
! --------------------------ZWJ----------------------------------------------
                 ...
                 zf = zgcc(jk)*pasya(ikl,ibm)**3
                 zwf = zomcc(jk)*zf
                 ztauc(jk) = (1.0_r8 - zwf) * ztauc(jk)
                 zomcc(jk) = (zomcc(jk) - zwf) / (1.0_r8 - zwf)
                 omegac1(jk) = 3._r8*(zgcc(jk)-zf)/(1.0_r8-zf)
                 omegac2(jk) = 5._r8*(zgcc(jk)*pasya(ikl,ibm)-zf)/(1.0_r8-zf)
                 omegac3(jk) = 7._r8*(zgcc(jk)*pasya(ikl,ibm)**2-zf)/(1.0_r8-zf)
               endif
! --------------------------ZWJ----------------------------------------------
! ---------------------------------------------------------------------------
...
! -----------------------------------ZWJ-----------------------------------------------------
                  ...
                  omegao1(jk) = (ptaucmc(ikl,iw) * pomgcmc(ikl,iw) * pasycmc(ikl,iw) + &
                              ptaua(ikl,ibm)*pomga(ikl,ibm)*pasya(ikl,ibm)) / zomco(jk)
                  omegao2(jk) = (ptaucmc(ikl,iw) * pomgcmc(ikl,iw) * pomega2mc(ikl,iw) + &
                              ptaua(ikl,ibm)*pomga(ikl,ibm)*pasya(ikl,ibm)**2) / zomco(jk)
                  omegao3(jk) = (ptaucmc(ikl,iw) * pomgcmc(ikl,iw) * pomega3mc(ikl,iw) + &
                              ptaua(ikl,ibm)*pomga(ikl,ibm)*pasya(ikl,ibm)**3) / zomco(jk)
                  zf = (ptaucmc(ikl,iw) * pomgcmc(ikl,iw) * pfsfcmc(ikl,iw) + &
                              ptaua(ikl,ibm)*pomga(ikl,ibm)*pasya(ikl,ibm)**4) / zomco(jk)
                  zomco(jk) = zomco(jk) / ztauo(jk)
                  zwf = zomco(jk) * zf
                  ztauo(jk) = (1._r8 - zwf) * ztauo(jk)
                  zomco(jk) = (zomco(jk) - zwf) / (1.0_r8 - zwf)
                  zgco (jk) = (zgco(jk) - zf) / (1.0_r8 - zf)
                  omegao1(jk) = 3.0 * (omegao1(jk) - zf) / (1.0 - zf)
                  omegao2(jk) = 5.0 * (omegao2(jk) - zf) / (1.0 - zf)
                  omegao3(jk) = 7.0 * (omegao3(jk) - zf) / (1.0 - zf)
                  end if
! -----------------------------------ZWJ-----------------------------------------------------
! -------------------------------------------------------------------------------------------
...
! ---------------------------------------------------------------------
! ---------------------------ZWJ---------------------------------------
             call reftra_sw (kmodts,klev, omegac1,omegac2,omegac3,&
                             lrtchkclr, zgcc, prmu0, ztauc, zomcc, &
                             zrefc, zrefdc, ztrac, ztradc,  &
                             zrdfc, ztdfc, zrdrc, ztdrc)
! ---------------------------ZWJ---------------------------------------
! ---------------------------------------------------------------------
...
! ---------------------------------------------------------------------
! ---------------------------ZWJ---------------------------------------
             call reftra_sw (kmodts,klev, omegao1,omegao2,omegao3,&
                             lrtchkcld, zgco, prmu0, ztauo, zomco, &
                             zrefo, zrefdo, ztrao, ztrado,  &
                             zrdfo, ztdfo, zrdro, ztdro)
! ---------------------------ZWJ---------------------------------------
! ---------------------------------------------------------------------
            do jk=1,klev
...
! --------------------------ZWJ----------------------------------
            if (kmodts==4) then
                  zrdr(jk,1:2)=zclear*zrdrc(jk,1:2)+zcloud*zrdro(jk,1:2)
                  zrdf(jk,1:2,1:2)=zclear*zrdfc(jk,1:2,1:2) &
                                +zcloud*zrdfo(jk,1:2,1:2)
                  ztdr(jk,1:2)=zclear*ztdrc(jk,1:2)+zcloud*ztdro(jk,1:2)
                  ztdf(jk,1:2,1:2)=zclear*ztdfc(jk,1:2,1:2) &
                                +zcloud*ztdfo(jk,1:2,1:2)
            else
! --------------------------ZWJ----------------------------------
! ---------------------------------------------------------------
...
! -----------------------------------------------------------------------
! ----------------------ZWJ----------------------------------------------
             call vrtqdr_sw (kmodts, klev, iw,  &
                             zrefc, zrefdc, ztrac, ztradc, &
                             zdbtc, zrdndc, zrupc, zrupdc, ztdbtc, &
                             zcd, zcu, zrdfc, ztdfc, zrdrc, ztdrc)
! ----------------------ZWJ----------------------------------------------
! -----------------------------------------------------------------------

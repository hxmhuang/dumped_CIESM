...
!--------------------------wmq-----------------------------
     use dim_readnl, only: nbandsw, nbandlw
!----------------------------------------------------------
...
!-------------------wmq-------------------
     type(var_desc_t) :: nswid
     type(var_desc_t) :: nlwid
!-----------------------------------------
...
!---------------wmq--------------------------
           case ('NBANDSW')
              ierr=pio_def_var (tape(t)%File, 'NBANDSW', PIO_DOUBLE, mdimids(i:i), nswid)
              str = 'SW band index'
              ierr=pio_put_att (tape(t)%File, nswid, 'long_name', trim(str))
              str = '1'
              ierr=pio_put_att (tape(t)%File, nswid, 'units', trim(str))
           case ('NBANDLW')
              ierr=pio_def_var (tape(t)%File, 'NBANDLW', PIO_DOUBLE, mdimids(i:i), nlwid)
              str = 'LW band index'
              ierr=pio_put_att (tape(t)%File, nlwid, 'long_name', trim(str))
              str = '1'
              ierr=pio_put_att (tape(t)%File, nlwid, 'units', trim(str))
!--------------------------------------------
           case default
...
!---------------wmq--------------------------
           case ('NBANDSW')
              ierr = pio_put_var (tape(t)%File, nswid, nbandsw)
           case ('NBANDLW')
              ierr = pio_put_var (tape(t)%File, nlwid, nbandlw)
!--------------------------------------------
           case default
              if(masterproc) write(iulog,*) 'Warning: no dimvar found for registered mdim named: ',hist_mdims(i)%name
           end select

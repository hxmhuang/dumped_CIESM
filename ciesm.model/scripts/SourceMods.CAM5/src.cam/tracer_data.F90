...
!----------------------------------------------------------------------- 
  ...
  ! !PUBLIC MEMBERS
  type input4d
     real(r8), dimension(:,:,:,:), pointer :: data_volc => null()
  endtype input4d
  ...
  type trfld
     real(r8), dimension(:,:,:), pointer :: data => null()
     real(r8), dimension(:,:,:,:), pointer :: data_volc => null()  !wmq
     type(input3d), dimension(4) :: input
     type(input4d), dimension(4) :: input_volc !wmq
  ...
  type trfile
     ...
     real(r8)                        :: p0
     type(var_desc_t) :: ps_id, cmip_id  !wmq
     ...
     logical :: cmip_data = .false. !wmq
     logical :: cmip_aod = .false. !wmq
     logical :: volc_sw = .false. !wmq
     logical :: volc_lw = .false. !wmq
  endtype trfile

  integer, public, parameter :: MAXTRCRS = 100

  integer, parameter :: MIDDIM = 1 !wmq
...
!wmq
    ierr = pio_inq_varid( file%curr_fileid, 'aod', file%cmip_id )
    file%cmip_data = (ierr==PIO_NOERR)
!do f = 1,mxnflds
!    if (flds(f)%srcnam .eq. trim('aod') ) then
!       file%cmip_aod = .true.
!    else
!       file%cmip_aod = .false.
!    end if
!write(iulog,*) 'f=',f
!write(iulog,*) 'flds(f)%srcnam=',flds(f)%srcnam
!write(iulog,*) 'file%cmip_aod=',file%cmip_aod
!end do
!---wmq--
do f = 1,mxnflds
    ierr = pio_inq_varid( file%curr_fileid, flds(f)%srcnam, flds(f)%var_id )

    ierr = pio_inq_dimid( file%curr_fileid, 'lw_band', idx )
    if (ierr==PIO_NOERR) then
        ierr = pio_inq_dimid( file%curr_fileid, 'lw_band', old_dimid)
        lw_dimid = old_dimid
        ierr = pio_inq_dimlen( file%curr_fileid, lw_dimid, file%nblw )
        ...
        file%volc_lw = any(vardimids(:nvardims)==lw_dimid)
        file%zonal_ave = .false.
    else
        file%volc_lw = .false.
    end if

    ierr = pio_inq_dimid( file%curr_fileid, 'sw_band', idx )
    if (ierr==PIO_NOERR) then
        ierr = pio_inq_dimid( file%curr_fileid, 'sw_band', old_dimid)
        sw_dimid = old_dimid
        ierr = pio_inq_dimlen( file%curr_fileid, sw_dimid, file%nbsw )
        ierr = pio_inquire_variable(  file%curr_fileid, flds(f)%var_id, ndims=nvardims )
        ierr = pio_inquire_variable(  file%curr_fileid, flds(f)%var_id, dimids=vardimids(:nvardims) )
        file%volc_sw = any(vardimids(:nvardims)==sw_dimid)
        file%zonal_ave = .false.
    else
        file%volc_sw = .false.
    end if
...
!--------
    ...
    flds_loop: do f = 1,mxnflds
       ...
       if ( .not. file%in_pbuf(f) ) then 
          ...
          !wmq
          if (file%volc_lw) then
               allocate( flds(f)%data_volc(pcols,pver,begchunk:endchunk,file%nblw), stat=astat   )
          else if (file%volc_sw) then
               allocate( flds(f)%data_volc(pcols,pver,begchunk:endchunk,file%nbsw), stat=astat   )
          end if
       ...
       else if(file%volc_lw) then !wmq
          allocate( flds(f)%input_volc(1)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nblw), stat=astat   )
       else if(file%volc_sw) then !wmq
          allocate( flds(f)%input_volc(1)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nbsw), stat=astat   )
       else
       ...
       if (flds(f)%srf_fld) then
          ...
          allocate( flds(f)%input_volc(2)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nblw), stat=astat   )
       else if(file%volc_sw) then !wmq
          allocate( flds(f)%input_volc(2)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nbsw), stat=astat   )
       else
       ...
       if( file%fill_in_months ) then
          if (flds(f)%srf_fld) then
             ...
             allocate( flds(f)%input_volc(3)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nblw), stat=astat   )
          else if(file%volc_sw) then !wmq
             allocate( flds(f)%input_volc(3)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nbsw), stat=astat   )
          else
          ...
          if (flds(f)%srf_fld) then
             ...
             allocate(flds(f)%input_volc(4)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nblw), stat=astat )
          else if(file%volc_sw) then !wmq
             allocate( flds(f)%input_volc(4)%data_volc(pcols,file%nlev,begchunk:endchunk,file%nbsw), stat=astat )
          else
       ...
       if ( file%zonal_ave ) then
          ...
          do did = 1,4
             if      ( dimids(did) == lw_dimid ) then
                flds(f)%coords(MIDDIM) = did
                flds(f)%order(did) = MIDDIM
             else if ( dimids(did) == lat_dimid ) then
                flds(f)%coords(LATDIM) = did
                flds(f)%order(did) = LATDIM
             else if ( dimids(did) == lev_dimid ) then
    ...
    do i=1,file%interp_recs
       ...
       do f = 1,nflds
          if ( file%zonal_ave ) then
             ...
             cnt4(flds(f)%coords(MIDDIM)) = file%nblw
             ...
             call read_lw_trc( fids(i), flds(f)%var_id, flds(f)%input_volc(i)%data_volc, strt4, cnt4, file, &
                  (/ flds(f)%order(MIDDIM),flds(f)%order(LATDIM),flds(f)%order(LEVDIM) /))
          else if (file%volc_sw) then !wmq
             cnt4(flds(f)%coords(MIDDIM)) = file%nbsw
             cnt4(flds(f)%coords(LATDIM)) = file%nlat
             cnt4(flds(f)%coords(LEVDIM)) = file%nlev
             cnt4(flds(f)%coords(TIMDIM)) = 1
             strt4(flds(f)%coords(TIMDIM)) = recnos(i)
             call read_sw_trc( fids(i), flds(f)%var_id, flds(f)%input_volc(i)%data_volc, strt4, cnt4, file, &
                  (/ flds(f)%order(MIDDIM),flds(f)%order(LATDIM),flds(f)%order(LEVDIM) /))
          else
...
!----wmq--
  subroutine read_lw_trc( fid, vid, loc_arr, strt, cnt, file, order )
    use interpolate_data, only : lininterp_init, lininterp, interp_type, lininterp_finish
    ...
    if(order(1)/=1 .or. order(2)/=2 .or. order(3)/=3 .or. &
         cnt(1)/=file%nblw.or.cnt(2)/=file%nlat.or.cnt(3)/=file%nlev) then
       allocate(wrk3d_in(file%nblw,file%nlat,file%nlev),stat=ierr)
       if( ierr /= 0 ) then
          write(iulog,*) 'read_3d_trc: wrk3d allocation error = ',ierr
          call endrun
       end if
       wrk3d_in = reshape( wrk3d(:,:,:),(/file%nblw,file%nlat,file%nlev/), order=order )
       deallocate(wrk3d)
    else
       wrk3d_in => wrk3d
    end if
    ...
    do c=begchunk,endchunk
       ...
       do ns=1,file%nblw
       do k=1,file%nlev
          call lininterp(wrk3d_in(ns,:,k), file%nlat, wrk(1:ncols), ncols, lat_wgts)
          loc_arr(1:ncols,k,c-begchunk+1,ns) = wrk(1:ncols)
       end do
       end do
       call lininterp_finish(lat_wgts)
    end do
  ...
  end subroutine read_lw_trc

  subroutine read_sw_trc( fid, vid, loc_arr, strt, cnt, file, order )
    use interpolate_data, only : lininterp_init, lininterp, interp_type, lininterp_finish
    use phys_grid,        only : pcols, begchunk, endchunk, get_ncols_p, get_rlat_all_p, get_rlon_all_p
    use mo_constants,     only : pi
    use dycore,           only : dycore_is
    ...
    if(order(1)/=1 .or. order(2)/=2 .or. order(3)/=3 .or. &
         cnt(1)/=file%nbsw.or.cnt(2)/=file%nlat.or.cnt(3)/=file%nlev) then
       allocate(wrk3d_in(file%nbsw,file%nlat,file%nlev),stat=ierr)
       if( ierr /= 0 ) then
          write(iulog,*) 'read_3d_trc: wrk3d allocation error = ',ierr
          call endrun
       end if
       wrk3d_in = reshape( wrk3d(:,:,:),(/file%nbsw,file%nlat,file%nlev/), order=order )
       deallocate(wrk3d)
    else
       wrk3d_in => wrk3d
    end if
...
!------------------------------------------------------------------------------
  ...
  subroutine interpolate_trcdata( state, flds, file, pbuf2d )
    ...
    real(r8) :: fact1, fact2
    real(r8) :: deltat
    integer :: f,nflds,c,ncol, i, k, ns, kk
    real(r8) :: ps(pcols)
    real(r8) :: datain(pcols,file%nlev)
    real(r8) :: pin(pcols,file%nlev)
    real(r8)            :: model_z(pverp)
    real(r8)            :: model_zi(pverp) !wmq
    real(r8), parameter :: m2km  = 1.e-3_r8
    real(r8), pointer :: data_out3d(:,:,:)
    real(r8), pointer :: data_out(:,:)
    real(r8), pointer :: data_out4d(:,:,:,:) !wmq 
    integer :: chnk_offset

    real(r8), allocatable ::  zlevs(:) !wmq
!    real(r8) ::  data_tmp(pcols,file%nlev) !wmq
    real(r8) ::  data_tmp(pcols,pverp) !wmq
...
!wmq
          do f = 1,nflds
             flds(f)%input_volc(1)%data_volc(:ncol,:,c,:) = fact1*flds(f)%input_volc(1)%data_volc(:ncol,:,c,:) &
                                                     + fact2*flds(f)%input_volc(3)%data_volc(:ncol,:,c,:)
          enddo
       enddo
...
!wmq
          do f = 1,nflds
             flds(f)%input_volc(2)%data_volc(:ncol,:,c,:) = fact1*flds(f)%input_volc(2)%data_volc(:ncol,:,c,:) &
                                                     + fact2*flds(f)%input_volc(4)%data_volc(:ncol,:,c,:)
          enddo
...
!wmq
       if (file%volc_lw .or. file%volc_sw) then
          data_out4d => flds(f)%data_volc(:,:,:,:)
       endif
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, PS, I, K, PIN, DATAIN, MODEL_Z, DATA_OUT)
       do c = begchunk,endchunk
          ...
          if (file%volc_lw) then
              do ns=1,file%nblw
                 data_out => data_out4d(:,:,c+chnk_offset,ns)
                   if (fact2 == 0) then  ! This needed as %data is not set if fact2=0 (and lahey compiler core dumps)
                       datain(:ncol,:) = fact1*flds(f)%input_volc(nm)%data_volc(:ncol,:,c,ns)
                   else
                       datain(:ncol,:) = fact1*flds(f)%input_volc(nm)%data_volc(:ncol,:,c,ns) &
                                       + fact2*flds(f)%input_volc(np)%data_volc(:ncol,:,c,ns)
                   end if
                   do i = 1,ncol
          ...
          if (file%alt_data) then !wmq1
            if (flds(f)%srf_fld) then
                ...
                do i = 1,ncol
                  ...
                  if (file%cmip_data) then

                     allocate(zlevs(file%nlev)) 
                      zlevs(1)=file%levs(1)+(file%levs(2)-file%levs(1))/2._r8
                      do k= 2,file%nlev
                         zlevs(k)=file%levs(k)+(file%levs(k)-file%levs(k-1))/2._r8
                      end do

                      if (flds(f)%srcnam .eq. trim('aod') ) then
                         do k= 1, pver
                            do kk = 1, file%nlev
                               if ( file%levs(kk).gt. model_z(k) .and. file%levs(kk).le. model_z(k+1) ) then
                                    data_tmp(i,k) = data_tmp(i,k) + datain(i,kk)
                               end if
                            end do
                         end do
                         ...
                         call rebin( file%nlev-1, pver, zlevs, model_z, datain(i,2:), data_out(i,:) )
                      end if
                      !write(iulog,*) 'file%levs=',file%levs
                      !write(iulog,*) 'model_zi=',model_zi
                      !call endrun
                      deallocate(zlevs)
                  else
                      call rebin( file%nlev, pver, file%ilevs, model_z, datain(i,:), data_out(i,:) )

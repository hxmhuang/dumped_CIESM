...
! ------------------ZWJ------------------
          ...
          call add_default('LIQ_ICLD_VISTAU', 1, ' ')
          call add_default('ICE_ICLD_VISTAU', 1, ' ')
! -----------------ZWJ------------------
! --------------------------------------
...
! --------------------------------------------------------------------------------------------------
! ------------------------------------------ZWJ-----------------------------------------------------
    real(r8) :: cld_tau_w_omega2(nbndsw,pcols,pver)
    real(r8) :: cld_tau_w_omega3(nbndsw,pcols,pver)
    real(r8) :: liq_tau_w_omega2(nbndsw,pcols,pver)
    real(r8) :: liq_tau_w_omega3(nbndsw,pcols,pver)
    real(r8) :: c_cld_tau_w_omega2(nbndsw,pcols,pver)
    real(r8) :: c_cld_tau_w_omega3(nbndsw,pcols,pver)
! ------------------------------------------ZWJ-----------------------------------------------------
! --------------------------------------------------------------------------------------------------
    ...
    if (dosw .or. dolw) then
       ...
       if (dosw) then
          if(oldcldoptics) then
             ...
             call slingo_liq_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, oldliqwp=.false.) ! ------------ZWJ-------------
!             call slingo_liq_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, liq_tau_w_omega2, liq_tau_w_omega3, oldliqwp=.false.)
          else
             ...
             case ('slingo')
                call slingo_liq_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, oldliqwp=.true.) ! -------------ZWJ------------
!                call slingo_liq_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, liq_tau_w_omega2, liq_tau_w_omega3, oldliqwp=.true.)
             case ('gammadist')
!                call slingo_liq_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, oldliqwp=.true.)
!                call slingo_liq_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, liq_tau_w_omega2, liq_tau_w_omega3, oldliqwp=.false.)
                call get_liquid_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f) ! ---------------ZWJ-------------
             case default
...
! ------------------------------------------------------------------------------------------------------
! --------------------------------------------------ZWJ-------------------------------------------------
!           cld_tau_w_omega2(:,1:ncol,:) =  liq_tau_w_omega2(:,1:ncol,:)
!           cld_tau_w_omega3(:,1:ncol,:) = liq_tau_w_omega3(:,1:ncol,:)
          do i = 1, nbndsw
           do j = 1,ncol
            do k = 1,pver
       if (liq_tau_w(i,j,k)>0._r8) then
              liq_tau_w_omega2(i,j,k) = liq_tau_w(i,j,k)*(liq_tau_w_g(i,j,k)/liq_tau_w(i,j,k))**2
              liq_tau_w_omega3(i,j,k) = liq_tau_w(i,j,k)*(liq_tau_w_g(i,j,k)/liq_tau_w(i,j,k))**3
              liq_tau_w_f(i,j,k) = liq_tau_w(i,j,k)*(liq_tau_w_g(i,j,k)/liq_tau_w(i,j,k))**4
            else
              liq_tau_w_omega2(i,j,k) =  0._r8
              liq_tau_w_omega3(i,j,k) =  0._r8
              liq_tau_w_f(i,j,k) = 0._r8
            end if
            if (ice_tau_w(i,j,k)>0._r8) then
              cld_tau_w_omega2(i,j,k) = liq_tau_w_omega2(i,j,k)+ice_tau_w(i,j,k)*(ice_tau_w_g(i,j,k)/ &
                                        ice_tau_w(i,j,k))**2
              cld_tau_w_omega3(i,j,k) = liq_tau_w_omega3(i,j,k)+ice_tau_w(i,j,k)*(ice_tau_w_g(i,j,k)/ &
                                        ice_tau_w(i,j,k))**3
              cld_tau_w_f(i,j,k) = liq_tau_w_f(i,j,k)+ice_tau_w(i,j,k)*(ice_tau_w_g(i,j,k)/ &
                                        ice_tau_w(i,j,k))**4
            else
              cld_tau_w_omega2(i,j,k) =  liq_tau_w_omega2(i,j,k)
              cld_tau_w_omega3(i,j,k) = liq_tau_w_omega3(i,j,k)
              cld_tau_w_f(i,j,k) = liq_tau_w_f(i,j,k)
            endif
            enddo
           enddo
          enddo
...
! -------------------------------------------------------------------------------------------------------
! --------------------------------------------------ZWJ--------------------------------------------------
!                      c_cld_tau_w_omega2(1:nbndsw,i,k)=cld_tau_w_omega2(1:nbndsw,i,k)
!                      c_cld_tau_w_omega3(1:nbndsw,i,k)=cld_tau_w_omega3(1:nbndsw,i,k)
                      do jj = 1, nbndsw
                        if(snow_tau_w(jj,i,k)>0._r8) then
                         c_cld_tau_w_omega2(jj,i,k)= &
                           (cldfsnow(i,k)*snow_tau_w(jj,i,k)*(snow_tau_w_g(jj,i,k)/ &
                           snow_tau_w(jj,i,k))**2 + &
                           cld(i,k)*cld_tau_w_omega2(jj,i,k))/cldfprime(i,k)
                         c_cld_tau_w_omega3(jj,i,k)= &
                           (cldfsnow(i,k)*snow_tau_w(jj,i,k)*(snow_tau_w_g(jj,i,k)/ &
                           snow_tau_w(jj,i,k))**3 + &
                           cld(i,k)*cld_tau_w_omega3(jj,i,k))/cldfprime(i,k)
                         c_cld_tau_w_f(jj,i,k)= &
                           (cldfsnow(i,k)*snow_tau_w(jj,i,k)*(snow_tau_w_g(jj,i,k)/ &
                           snow_tau_w(jj,i,k))**4 + &
                           cld(i,k)*cld_tau_w_f(jj,i,k))/cldfprime(i,k)
                        else
                         c_cld_tau_w_omega2(jj,i,k)= cld_tau_w_omega2(jj,i,k)
                         c_cld_tau_w_omega3(jj,i,k)= cld_tau_w_omega3(jj,i,k)
                         c_cld_tau_w_f(jj,i,k)= cld_tau_w_f(jj,i,k)
                        endif
                      enddo
! --------------------------------------------------ZWJ--------------------------------------------------
! -------------------------------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------------------------------
! --------------------------------------------------ZWJ-------------------------------------------------
                      c_cld_tau_w_omega2(1:nbndsw,i,k)= 0._r8
                      c_cld_tau_w_omega3(1:nbndsw,i,k)= 0._r8
! --------------------------------------------------ZWJ-------------------------------------------------
! ------------------------------------------------------------------------------------------------------
                   endif
                enddo
...
! -------------------------------------------------------------------------------------------------------
! --------------------------------------------------ZWJ--------------------------------------------------
             c_cld_tau_w_omega2(1:nbndsw,1:ncol,:) = cld_tau_w_omega2(:,1:ncol,:)
             c_cld_tau_w_omega3(1:nbndsw,1:ncol,:) = cld_tau_w_omega3(:,1:ncol,:)
! --------------------------------------------------ZWJ--------------------------------------------------
! -------------------------------------------------------------------------------------------------------
          endif
       endif
...
! -------------------------------------------------------ZWJ------------------------------------------------
                  call rad_rrtmg_sw( &
                       ...
                       su,           sd, &
                       E_cld_tau=c_cld_tau, E_cld_tau_w=c_cld_tau_w, E_cld_tau_w_g=c_cld_tau_w_g, E_cld_tau_w_f=c_cld_tau_w_f, &
                       E_cld_tau_w_omega2=c_cld_tau_w_omega2, E_cld_tau_w_omega3=c_cld_tau_w_omega3, old_convert = .false.)
! -------------------------------------------------------ZWJ------------------------------------------------
! ----------------------------------------------------------------------------------------------------------

...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
	  ...
	  case ('canuto')
         vmix_itype = vmix_type_canuto
         write(stdout,'(a32)') '  Using Canuto vertical mixing     '
      case default
         vmix_itype = -1000
      end select
...
!-----------------------------------------------------------------------
   ...
   case(vmix_type_kpp)
	...
	case(vmix_type_canuto)
	   allocate (VDC(nx_block,ny_block,km,2,nblocks_clinic), &
                VVC(nx_block,ny_block,km,      nblocks_clinic))
       call get_timer(timer_vmix_coeffs,'VMIX_COEFFICIENTS_canuto', &
                                  nblocks_clinic, distrb_clinic%nprocs)
   end select
...
!-----------------------------------------------------------------------
   ...
   if (vmix_itype == vmix_type_kpp .or. vmix_itype == vmix_type_canuto) then
...
!-----------------------------------------------------------------------
   ...
   case(vmix_type_canuto)
      if (k == 1) then     ! for KPP, compute coeffs for all levels
         ...
         if (present(SMFT)) then
            call vmix_coeffs_canuto(VDC(:,:,:,:,bid),           &
                                 VVC(:,:,:,  bid),           &
                                 TMIX,UMIX,VMIX,RHOMIX, &
                                 STF,SHF_QSW,                &
                                 this_block,                 &
...
!CDIR COLLAPSE
      ...
      do i=ib,ie
		 ...
		 ! if((mt2.eq.1).and.(TNEW(i,j,k,1).gt.40))then
		 
          ! write(10,*), "lat,lon,k=",TLATD(i,j,bid),TLOND(i,j,bid),k
		  ! write(10,*), "TOLD,TNEW,F=",TOLD(i,j,k,n),TNEW(i,j,k,n),F(i,j,k)
		  ! write(10,*), "VDC=",VDC(i,j,:,mt2,bid)

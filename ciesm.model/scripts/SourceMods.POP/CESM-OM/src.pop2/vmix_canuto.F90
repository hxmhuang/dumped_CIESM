module vmix_canuto
    ...
    real (kind=r8) ::           &
	  ...
	  bckgrnd_vdc2,       &! background diffusivity (Ledwell)  
...
!-----------------------------------------------------------------------
!
!  compute bckgrnd_vdc and bckgrnd_vvc
!
!-----------------------------------------------------------------------	
    if ( ltidal_mixing ) TIDAL_DIFF = c0
	...
	if ( ltidal_mixing ) then
        ...
        where (AN2_XY(:,:,k) > c0)
         TIDAL_DIFF(:,:,k) = TIDAL_COEF(:,:,k,bid)/max(AN2_XY(:,:,k),1.0d-8)
        endwhere
	
	    bckgrnd_vdc(:,:,k) = min(bckgrnd_vdc(:,:,k) + TIDAL_DIFF(:,:,k),  &
                               tidal_mix_max)
...
!-----------------------------------------------------------------------
	...
	do jj=1,ny_block
		...
		vvc(ii,jj,:)=akm+bckgrnd_vvc(ii,jj,:)
		vdc(ii,jj,:,1)=akh+bckgrnd_vdc(ii,jj,:)
		vdc(ii,jj,:,2)=aks+bckgrnd_vdc(ii,jj,:)

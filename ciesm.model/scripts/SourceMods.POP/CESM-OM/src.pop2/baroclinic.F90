...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::  &
      i,j,ii,                &! dummy indices for horizontal directions
...
!-----------------------------------------------------------------------
   ...
   !$OMP END PARALLEL DO
 !  open(unit=113,position='Append',file="istep.txt")
...
!-----------------------------------------------------------------------
   ...
   !$OMP END PARALLEL DO
    ! write(112,*)"Max(T)" ,maxval(TRACER (:,:,:,1,mixtime,:))
	! write(112,*)"Max(S)" ,maxval(TRACER (:,:,:,2,mixtime,:))
	! write(112,*)"Min(T)" ,minval(TRACER (:,:,:,1,mixtime,:))
	! write(112,*)"Min(S)" ,minval(TRACER (:,:,:,2,mixtime,:))
	! write(112,*)"Max(U)",maxval(UVEL(:,:,:,mixtime,:))
	! write(112,*)"Min(U)",minval(UVEL(:,:,:,mixtime,:))
    ! write(112,*)"Max(V)",maxval(VVEL(:,:,:,mixtime,:))
    ! write(112,*)"Min(V)",minval(VVEL(:,:,:,mixtime,:))

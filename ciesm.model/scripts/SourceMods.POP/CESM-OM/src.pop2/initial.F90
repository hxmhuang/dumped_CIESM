...
! !USES:
   ...
   use vertical_mix, only: init_vertical_mix, vmix_itype, vmix_type_kpp,vmix_type_canuto
...
!-----------------------------------------------------------------------
   ...
   if (check_all(ltidal_mixing .and. vmix_itype /= vmix_type_kpp .and. vmix_itype /= vmix_type_canuto)) then
     exit_string =   &
     'FATAL ERROR:  Tidally driven mixing is only allowed when KPP mixing or canuto mixing is enabled' 
     call document ('POP_check', exit_string)
     number_of_fatal_errors = number_of_fatal_errors + 1
   endif
...
!-----------------------------------------------------------------------
   ...
   if (check_all(lniw_mixing .and. vmix_itype /= vmix_type_kpp .and. vmix_itype /= vmix_type_canuto)) then
     exit_string =   &
     'FATAL ERROR:  Near-inertial wave mixing is only allowed when KPP mixing or canuto mixing is enabled' 
     call document ('POP_check', exit_string)
     number_of_fatal_errors = number_of_fatal_errors + 1
   endif

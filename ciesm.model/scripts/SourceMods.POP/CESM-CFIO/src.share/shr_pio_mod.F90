...
!<
  subroutine shr_pio_init1(ncomps, nlfilename, GLOBAL_COMM)
    ...
    integer :: mpigrp_world, mpigrp, ierr, mpicom, compute_com
    character(*),parameter :: subName =   '(shr_pio_init1) '
    integer :: pelist(3,1)
    integer :: size_proc, rank_proc, group_comp 
...
#ifdef CFIO_USED
!    call mpi_comm_size(GLOBAL_COMM, size_proc, ierr)
!    call mpi_comm_size(GLOBAL_COMM, rank_proc, ierr)
!
!    pelist(1, 1) = size_proc - CFIO_IO_NUM
!    pelist(2, 1) = size_proc - 1
!    pelist(3, 1) = 1
!
!    call mpi_comm_group(GLOBAL_COMM, mpigrp_world, ierr)
!    call shr_mpi_chkerr(ierr,subname//' mpi_comm_group mpigrp_world')
!    call mpi_group_range_incl(mpigrp_world, 1, pelist, group_io,ierr)
!    call shr_mpi_chkerr(ierr,subname//' mpi_group_range_incl group_io')
!    call mpi_comm_create(GLOBAL_COMM, group_io, io_comm, ierr)
!
!    call mpi_group_range_excl(mpigrp_world, 1, pelist, group_comp, ierr)
!    call shr_mpi_chkerr(ierr,subname//' mpi_group_range_incl group_comp')
!    call mpi_comm_create(GLOBAL_COMM, group_comp, mpicom, ierr)
!    GLOBAL_COMM=mpicom
!
!    write(*, *) rank_proc, mpigrp_world, group_io, group_comp, GLOBAL_COMM, io_comm
#endif
       ...
       do i=1,total_comps
	     ...
	     if(len_trim(cname) <= 3) then
             ...
             call pio_init(comp_comm_iam(i), comp_comm(i), pio_comp_settings(i)%pio_numiotasks, 0, &
                  ...
                  pio_rearr_box, iosystems(i), &  !origin piorearr_box 
                  !pio_rearr_none, iosystems(i), &  
...
!       iotype = pio_iotype_vdc
       ...
       iotype = defaulttype
!>>>>>>>>>>>>>>>>>>>>>>>>>
    else if ( typename .eq. 'CFIO') then
        iotype = pio_iotype_cfio
!<<<<<<<<<<<<<<<<<<<<<<<<
    else

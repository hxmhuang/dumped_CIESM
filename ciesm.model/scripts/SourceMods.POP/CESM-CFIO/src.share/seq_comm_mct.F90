...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
!  integer, parameter :: ncomps = (2 + 2*nphysmod + (3 * num_inst_phys))
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
!  integer, public :: ATMIOID(num_inst_atm)
!  integer, public :: LNDIOID(num_inst_atm)
!  integer, public :: OCNIOID(num_inst_atm)
!  integer, public :: ICEIOID(num_inst_atm)
!  integer, public :: GLCIOID(num_inst_atm)
!  integer, public :: ROFIOID(num_inst_atm)
!  integer, public :: WAVIOID(num_inst_atm)
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  type(seq_comm_type), public :: seq_comms(ncomps)
...
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    do n = 1,ncomps
       ...
       seq_comms(n)%cmppe = -1
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       seq_comms(n)%iocom = MPI_COMM_NULL
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    enddo
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       call seq_comm_setiocomm(ATMID(n), pelist, atm_nthreads, 'ATMIO', n, &
       num_inst_atm, tmppe, CFIO_ATMIO_NUM)  
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       call seq_comm_setiocomm(LNDID(n), pelist, lnd_nthreads, 'LNDIO', n, &
       num_inst_lnd, tmppe, CFIO_LNDIO_NUM)  
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       call seq_comm_setiocomm(OCNID(n), pelist, ocn_nthreads, 'OCNIO', n, &
       num_inst_ocn, tmppe, CFIO_OCNIO_NUM)  
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
 
       call seq_comm_joincomm(CPLID, OCNID(n), CPLOCNID(n), 'CPLOCN', n, num_inst_ocn)
    end do
    !print *, "[wwj iocom],", mype,seq_comms(OCNID(1))%iocom 
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       call seq_comm_setiocomm(ICEID(n), pelist, ice_nthreads, 'ICEIO', n, &
       num_inst_ice, tmppe, CFIO_ICEIO_NUM)  
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       call seq_comm_setiocomm(GLCID(n), pelist, glc_nthreads, 'GLCIO', n, &
       num_inst_glc, tmppe, CFIO_GLCIO_NUM)  
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       call seq_comm_setiocomm(ROFID(n), pelist, rof_nthreads, 'ROFIO', n, &
       num_inst_rof, tmppe, CFIO_ROFIO_NUM)  
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       call seq_comm_setiocomm(WAVID(n), pelist, wav_nthreads, 'WAVIO', n, &
       num_inst_wav, tmppe, CFIO_WAVIO_NUM)  
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
    ...
    if(mype == 0) write(*,*) "here1"
    call mct_world_init(ncomps, GLOBAL_COMM, comms, comps)

    if(mype == 0) write(*,*) "here2"
    deallocate(comps,comms)

    call seq_comm_printcomms()

    if(mype == 0) write(*,*) "here3"

  end subroutine seq_comm_init
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
!---------------------------------------------------------
  subroutine seq_comm_setiocomm(ID,pelist,nthreads,iname,inst,tinst,tmppe,ionum)
    ...
    integer :: tmppelist(3,1)       ! start, stop, stride for group
    integer :: mpigrp_world
    integer :: clientgrp, servergrp, iogrp,grpsize
    integer :: iocom,ionum,tmppe,compcom
    integer :: ntask,ntasks,cnt
    integer :: ierr
    character(len=seq_comm_namelen) :: cname
    logical :: set_suffix
    character(*),parameter :: subName =   '(seq_comm_setiocomm) '
    ...
    !if (ID == OCNID(1)) then
    !        print *, "[wwj mpigrp_world]", global_mype, mpigrp_world
    !endif
    call shr_mpi_chkerr(ierr,subname//' mpi_comm_group mpigrp_world')
    call mpi_group_range_incl(mpigrp_world, 1, pelist, clientgrp,ierr)
    !if (ID == OCNID(1)) then
    !        print *, "[wwj clientgrp]", global_mype, clientgrp
    !endif
    call shr_mpi_chkerr(ierr,subname//' mpi_group_range_incl mpigrp')
!    call mpi_comm_create(GLOBAL_COMM, clientgrp, compcom, ierr)
!    call shr_mpi_chkerr(ierr,subname//' mpi_comm_create mpigrp')
    if (ionum /= 0) then
        tmppelist(1,1) = tmppe
        tmppelist(2,1) = tmppe + ionum - 1
        tmppelist(3,1) = 1
    
        call mpi_group_range_incl(mpigrp_world, 1, tmppelist, servergrp,ierr)
         
    !if (ID == OCNID(1))then
    !    print *, "[wwj servergrp]", global_mype, servergrp
    !    call MPI_GROUP_SIZE(servergrp, grpsize, ierr)
    !    print *,"[wwh server grpsize]", global_mype,grpsize
    !endif
        call shr_mpi_chkerr(ierr,subname//' mpi_group_range_incl mpigrp')
        call mpi_group_union(clientgrp, servergrp, iogrp, ierr)
    !if (ID == OCNID(1))then
    !    print *, "[wwj iogrp]", global_mype, iogrp
    !    call MPI_GROUP_SIZE(iogrp, grpsize, ierr)
    !    print *,"[wwh io grpsize]", global_mype,grpsize
    !endif
        call shr_mpi_chkerr(ierr,subname//' mpi_group_range_incl mpigrp')
        call mpi_comm_create(GLOBAL_COMM, iogrp, iocom, ierr)
    !if (ID == OCNID(1)) then
    !    print *, "[wwj iocom]", global_mype, iocom
    !    CALL MPI_COMM_SIZE(iocom, grpsize, ierr)
    !    print *,"[wwh iocom size]", global_mype,grpsize
    !endif
        call shr_mpi_chkerr(ierr,subname//' mpi_comm_create mpigrp')
    endif
    if (ionum == 0) then
        iocom = MPI_COMM_NULL
        iogrp = MPI_GROUP_NULL
    endif
...
!    endif
   ...
   seq_comms(ID)%iocom = iocom
   seq_comms(ID)%iogrp = iogrp
   seq_comms(ID)%ionum = ionum
!   seq_comms(ID)%mpicom = compcom
!   seq_comms(ID)%mpigrp = clientgrp
...
!    endif
!   
!    if (iocom /= MPI_COMM_NULL) then
!       call mpi_comm_size(iocom,seq_comms(ID)%npes,ierr)
!       call shr_mpi_chkerr(ierr,subname//' mpi_comm_size')
!       call mpi_comm_rank(iocom,seq_comms(ID)%iam,ierr)
...
!
  end subroutine seq_comm_setiocomm

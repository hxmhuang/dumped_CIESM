...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
   ...
   integer  :: mpicom_ATMIOID(num_inst_atm)           ! MPI atm io communicator
   integer  :: mpicom_LNDIOID(num_inst_lnd)           ! MPI lnd io communicator
   integer  :: mpicom_ICEIOID(num_inst_ice)           ! MPI ice io communicator
   integer  :: mpicom_OCNIOID(num_inst_ocn)           ! MPI ocn io communicator
   integer  :: mpicom_GLCIOID(num_inst_glc)           ! MPI glc io communicator
   integer  :: mpicom_ROFIOID(num_inst_rof)           ! MPI rof io communicator
   integer  :: mpicom_WAVIOID(num_inst_wav)           ! MPI wav io communicator
...
!       !! !!This is io proc
!       write (*,*) "myrank", myrank, "I am an io process"
!       do while (.true.) 
!	   myrank = 1
!       end do
!   endif
...
!      call mpi_group_rank(seq_comms(OCNID(1))%iogrp, part_rank1, ierr)
!      if(part_rank1 == MPI_UNDEFINED) then
!          write (*, *) "wwj , this pes is not in pop io"     
!      endif
!      if (mype == 0) then 
!          write(*, *) "zero pes is here"
!      endif
!      if(mype == 240) then 
!          write(*, *) "240 pes is here"
!      endif
!      if (mype >= CFIO_CLIENT_NUM) then
!          do while (.true.) 
!              continue 
!          enddo
!      endif
 ...
 106  format( A, f23.12)
   ...
   do eai = 1,num_inst_atm
      ...
      if (iamin_ATMID(eai) .and. atm_present) then
         ...
         call shr_sys_flush(logunit)
         !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> zc
         !print *, __FILE__, __LINE__, "zc pio atm_init_mct"
         !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< zc
...
!tcx fails     call map_atm2ice_mct( cdata_ax, a2x_ax(eai), cdata_ix, a2x_ix(eai) )
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwj river runoff run ", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      if (rof_present .and. rofrun_alarm) then
         do eri = 1,num_inst_rof
...
!            call seq_hist_spewav('x2a_ax(eai)',gsmap_ax,x2a_ax(eai),atm_nx,atm_ny,1)
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwjj save driver ", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      if ( restart_alarm .and. iamin_CPLID) then
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwjj save driver 1 ", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
         if (run_barriers) then
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwjj save driver 2 ", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwjj save driver 3 ", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
         if (iamroot_CPLID) then
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwjj save driver 4 ", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
         call seq_rest_write(EClock_d,seq_SyncClock)
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwjj save driver 5 ", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
         if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      !if(mype == 0) then
      !    print *, "wwjj  write history", iter1
      !endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      if (iamin_CPLID) then

...
!
      ...
      real (kind=dbl_kind), parameter :: &
         ...
         betaS     = p25            ,&! power exponent appearing in astar and 
         LminS     = c4             ,&! min length of floe (m) [5,100], Antarctica
         LmaxS     = 150._dbl_kind    ! max length of floe (m) [30,3000]

      real (kind=dbl_kind) :: cfloediam,astar,astarS
      
      astar  = c1/(c1-(Lmin/Lmax)**(c1/beta))
      astarS = c1/(c1-(LminS/LmaxS)**(c1/betaS))
...
!!!         fbot(i,j) = min (c0, frzmlt(i,j))
      ...
      !-----------------------------------------------------------------
         ...
         if (tlat(i,j) > 0) then
             cfloediam = Lmin * (astar / (astar - aice(i,j)))**beta
         else 
             cfloediam = LminS * (astarS / (astarS - aice(i,j)))**betaS
         endif
         rside(i,j) = wlat*dt*pi/(alpha*cfloediam) ! XSM

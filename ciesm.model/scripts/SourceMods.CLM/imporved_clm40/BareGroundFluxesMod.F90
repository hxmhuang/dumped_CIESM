...
!
    ...
    ! new added
    real(r8) , parameter :: p0     = 1.0e5
    real(r8) , parameter :: rddcp  = 0.285857
    real(r8) :: pt1, nu
!------------------------------------------------------------------------------
    ...
    do iter = 1, niters
       ...
       do f = 1, fn
          ...
          ! Yang---------------------------------------------------------------
          ! ---------
          pt1  = forc_t(c)*(forc_pbot(c)/(forc_pbot(c)-forc_rho(c)*9.81*(zldis(p) + displa(p))))**rddcp ! 2m height air temperature
          nu   = 1.328e-5*(p0/forc_pbot(c))* (pt1/273.15)**1.754
          z0hg_pft(p) = 70 * nu / ustar(p) *exp(-7.2*sqrt(ustar(p))*sqrt(sqrt(abs(-tstar))))
          z0hg_pft(p) = min((zldis(p) +displa(p))/10.,max(z0hg_pft(p),1.0E-10))

...
!-----------------------------------------------------------------------
 !  save
   ...
   character (88) ,save :: line ! temporary for line of text input/output
...
!-----------------------------------------------------------------------
  ! save
   ...
   logical (log_kind), dimension(:), allocatable, save  :: post_array
   integer(i4), dimension(:), allocatable, save  :: myRequests(:)
   integer (i4), dimension(:,:), allocatable, save:: myStati(:,:)

   integer (int_kind), parameter :: len = 14 !length of buffer for a post

   real (r8), dimension(:), allocatable, save :: myRecvBuff(:), mySendBuff(:)
   ...
   character (10), save        :: &! for input year,month,day
      cdate

   logical (log_kind), save :: post
...
!
   ...
   character (POP_charLength), save ::  &
     string

   integer (POP_i4), save ::  &
     ier

   integer (int_kind), save  :: ovf_id


   integer (int_kind), save ::  num_posts, ierr, count, loc, r_n, r_m, indx
   integer (int_kind), parameter :: len = 16 !length of buffer for a post
   real (r8), dimension(:), allocatable, save :: myRecvBuff(:), mySendBuff(:)


   logical (log_kind), save :: post, found
   logical (log_kind), dimension(:), allocatable, save :: post_array

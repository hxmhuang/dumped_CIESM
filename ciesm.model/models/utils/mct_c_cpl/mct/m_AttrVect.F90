...
! !PUBLIC TYPES:
    ...
    type AttrVect
      ...
      integer,dimension(:,:),pointer :: ifield
      real(FP),dimension(:,:),pointer :: rfield
      integer,dimension(:),pointer :: field_ids
      character(1024) :: name
      integer :: comp_id
    end type AttrVect
...
! !PUBLIC MEMBER FUNCTIONS:
    ...
    !edited by Zhang Cheng
    interface import; module procedure import_ ; end interface
    interface export; module procedure export_ ; end interface
...
!
  ...
  allocate(av%ifield(n,nIA),av%rfield(n,nRA+1),av%field_ids(nIA+nRA+1),stat=ier)
  if(ier /= 0) call die(myname_,'allocate()',ier)
...
!EOP ___________________________________________________________________
  ...
  !edited by Zhang Cheng
  if(associated(aV%ifield)) then

      deallocate(aV%ifield,stat=ier)

     if(ier /= 0) then
	if(present(stat)) then
	   stat=ier
	else
        call warn(myname_,'deallocate(aV%ifield)',ier)
	endif
     endif

 endif ! if(associated(aV%ifield))...

  if(associated(aV%rfield)) then

      deallocate(aV%rfield,stat=ier)

     if(ier /= 0) then
	if(present(stat)) then
	   stat=ier
	else
        call warn(myname_,'deallocate(aV%rfield)',ier)
	endif
     endif

 endif ! if(associated(aV%rfield))...

  if(associated(aV%field_ids)) then

      deallocate(aV%field_ids,stat=ier)

     if(ier /= 0) then
	if(present(stat)) then
	   stat=ier
	else
        call warn(myname_,'deallocate(aV%field_ids)',ier)
	endif
     endif
...
!EOP ___________________________________________________________________
  ...
  subroutine export_(avect, fldlist)
      use m_List,          only : GetIndices => get_indices
      ...
      integer :: i, j, numi, numr, lsize
      integer, dimension(:), pointer :: iAttr_indices, rAttr_indices
      ...
      if (present(fldlist)) then
          !call GetIndices(iAttr_indices, avect%iList, trim(fldlist))
          !numi = size(iAttr_indices)

          !do i = 1, numi
          !    avect%ifield(:,iAttr_indices(i)) = avect%iAttr(iAttr_indices(i),:)
          !end do

          !if (associated(iAttr_indices)) deallocate(iAttr_indices)

          call GetIndices(rAttr_indices, avect%rList, trim(fldlist))
          numr = size(rAttr_indices)

          do i = 1, numr
              do j = 1, lsize
                  avect%rfield(j,rAttr_indices(i)) = avect%rAttr(rAttr_indices(i),j)
              end do
          end do

          if (associated(rAttr_indices)) deallocate(rAttr_indices)
      else
          numi = nIAttr_(avect)
          numr = nRAttr_(avect)
          ...
          do i = 1, numi
              do j = 1, lsize
                  avect%ifield(j,i) = avect%iAttr(i,j)
              end do
          end do

          do i = 1, numr
              do j = 1, lsize
                  avect%rfield(j,i) = avect%rAttr(i,j)
              end do
          end do
      end if

  end subroutine export_

  subroutine import_(avect, fldlist)
      use m_List,          only : GetIndices => get_indices
      ...
      if (present(fldlist)) then
          ...
          !do i = 1, numi
          !    avect%iAttr(iAttr_indices(i),:) = avect%ifield(:,iAttr_indices(i))
          !end do
          ...
          do i = 1, numr
              do j = 1, lsize
                  avect%rAttr(rAttr_indices(i),j) = avect%rfield(j,rAttr_indices(i))
              end do
          end do
          ...
          do i = 1, numi
              do j = 1, lsize
                  avect%iAttr(i,j) = avect%ifield(j,i)
              end do
          end do

          do i = 1, numr
              do j = 1, lsize
                  avect%rAttr(i,j) = avect%rfield(j,i)
              end do
          end do
      end if

  end subroutine import_

...
!!
!! $Revision: 766 $
!! $LastChangedDate: 2013-04-23 06:42:31 +0800 (Tue, 23 Apr 2013) $
!!
!<
module nf_mod
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       ...
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq not defined"
          endif
#endif
          ierr = nfmpi_inq( File%fh,vals(1),vals(2), &
               vals(3),vals(4))
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_att not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_att ... ", name(1:nlen)
          endif
#endif
          !ierr=nfmpi_inq_att(File%fh,varid,name(1:nlen),xtype,clen)
          !ierr = PIO_NOERR
          ierr = cfio_inq_att(File%fh, varid, name(1:nlen) )
          len = INT(clen,kind=i4)

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_att over ", name(1:nlen), ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_attlen not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_attlen not defined"
          endif
#endif
          ierr=nfmpi_inq_attlen(File%fh,varid,name(1:nlen),int(clen))
          len = INT(clen,kind=i4)

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_attlen over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_attname not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_attname not defined"
          endif
#endif
          ierr=nfmpi_inq_attname(File%fh,varid,attnum,tmpname)
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_attname over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       !   open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       !   write(1024, *) __FILE__, __LINE__, "cfio_inq_varid not defined in cfio! " 
       !   close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_varid ... ", name(1:nlen)
          endif
#endif
          ...
          !!! CFIO
          if (ierr /= PIO_NOERR) ierr = cfio_inq_varid(File%fh,name(1:nlen),varid)
          !ierr = PIO_NOERR

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_varid over ", name(1:nlen), ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_varname not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_varname not defined"
          endif
#endif
          ierr=nfmpi_inq_varname(File%fh,varid,name(1:nlen))
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_varname over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_varndims not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_varndims ... "
          endif
#endif
          ierr=nfmpi_inq_varndims(File%fh,varid,ndims)

          if (PIO_NOERR /= ierr) ierr = cfio_inq_varndims(File%fh,varid,ndims)

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_varndims over ", ndims, ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_vartype not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_vartype not defined"
          endif
#endif
          ierr=nfmpi_inq_vartype(File%fh,varid,type)
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_vartype over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
        !open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
        !write(1024, *) __FILE__, __LINE__, "cfio_inq_vardimid not defined in cfio! " 
        !close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_vardimid ... "
          endif
#endif
          ierr=nfmpi_inq_vardimid(File%fh,varid,dimids)

          if (PIO_NOERR /= ierr) ierr = cfio_inq_vardimid(File%fh, varid, dimids)

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_vardimid over ", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_varnatts not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_varnatts not defined"
          endif
#endif
          ierr=nfmpi_inq_varnatts(File%fh,varid,natts)
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_varnatts over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_dimid not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_dimid ... ", name(1:nlen)
          endif
#endif
          !!! for read nc, client use PnetCDF
          ierr=nfmpi_inq_dimid(File%fh,name(1:nlen),dimid)

          !!! for write nc, client use CFIO
          if(ierr /= PIO_NOERR) ierr = cfio_inq_dimid(File%fh,name(1:nlen),dimid)

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_dimid over ", name(1:nlen), ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_dimname not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_dimname not defined"
          endif
#endif
          ierr=nfmpi_inq_dimname(File%fh,dimid,dimname(1:ldn))
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_dimname over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_inq_dimlen not defined in cfio! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_inq_dimlen not defined"
          endif
#endif
          ierr=nfmpi_inq_dimlen(File%fh,dimid,clen)
          dimlen = INT(clen,kind=i4)
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_inq_dimlen over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       ! open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       ! write(1024, *) __FILE__, __LINE__, "cfio_enddef - nfmpi_enddef! " 
       ! close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_enddef ... "
          endif
#endif
          !!! PnetCDF
          !ierr=nfmpi_enddef(File%fh)
          ...
          !!! CFIO
          ierr = cfio_enddef(File%fh)
          ierr = PIO_NOERR

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_enddef over ", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       !   open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       !   write(1024, *) __FILE__, __LINE__, "cfio_redef not defined in cfio! " 
       !   close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_redef not defined"
          endif
#endif
          ierr=nfmpi_redef(File%fh)
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_redef over", ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       !   open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       !   write(1024, *) __FILE__, __LINE__, "cfio_def_dim - nfmpi_defdim! " 
       !   close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_def_dim ... ", name(1:nlen)
          endif
#endif
          clen = len
          ...
          !!! CFIO
          ierr = cfio_def_dim(File%fh, name(1:nlen), len, dimid)
          ierr = PIO_NOERR

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_def_dim over ", name(1:nlen), ierr
          endif
#endif
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
    integer, dimension(2) :: st, cnt
    integer, dimension(:) ,allocatable:: nst, ncnt
    integer :: n
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       ...
       case(pio_iotype_cfio)
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
           !allocate(nst( vardesc%ndims))
           !allocate(ncnt( vardesc%ndims))
           !st(1) = 3
           !st(2) = 3
           !cnt(1) = POP_BLCKX
           !cnt(2) = POP_BLCKY
           !do n=1,vardesc%ndims
           !     nst(n) = st(n)
           !     ncnt(n) = cnt(n)
           !enddo
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
#ifdef CFIO_DEBUG
       !   open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       !   write(1024, *) __FILE__, __LINE__, "cfio_def_var - nfmpi_def_var! " 
       !   close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_def_var ... ", name(1:nlen)
          endif
#endif
          if(vardesc%ndims==0) then
             !ierr=nfmpi_def_var(File%fh,name(1:nlen),type,vardesc%ndims,dimids,vardesc%varid)

             !ierr=cfio_def_var(File%fh,name(1:nlen),type,vardesc%ndims,dimids,nst,ncnt,vardesc%varid)
             ierr = cfio_def_var(File%fh,name(1:nlen),type,vardesc%ndims,dimids,vardesc%varid)
             !ierr = PIO_NOERR
             !continue
             ...
             !ierr=cfio_def_var(File%fh,name(1:nlen),type,vardesc%ndims,dimids(1:vardesc%ndims),nst,ncnt,vardesc%varid)
             ierr = cfio_def_var(File%fh,name(1:nlen),type,vardesc%ndims,dimids(1:vardesc%ndims),vardesc%varid)
             !ierr = PIO_NOERR
             !continue
          end if
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_def_var over ", name(1:nlen), "varid=", vardesc%varid, ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       !   open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       !   write(1024, *) __FILE__, __LINE__, "cfio_copy_att not defined in cfio! " 
       !   close(1024)
          if (infile%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_copy_att not defined"
          endif
#endif
          ierr = nfmpi_copy_att(infile%fh, invarid, name, &
               outfile%fh, outvarid)
...
#ifdef CFIO_DEBUG
          if (infile%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_copy_att over", ierr
          endif
#endif

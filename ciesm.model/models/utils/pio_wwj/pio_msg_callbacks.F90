...
subroutine pio_callback_handler(iosystem, msg)
  ...
  case (PIO_MSG_CLOSE_FILE)
     ...
     deallocate(file)
        ...
        write(1024, *) __FILE__, __LINE__, "[wwj]: callback_handle call closefile " 
        close(1024)
        !print *,__FILE__,__LINE__,"[wwj]: callback_handle call closefile"
#endif
...
#ifdef CFIO_DEBUG
        open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
        write(1024, *) __FILE__, __LINE__, "[wwj]: callback_handle call syncfile " 
        close(1024)
        !print *,__FILE__,__LINE__,"[wwj]: callback_handle call syncfile"
#endif
  case default
...
#ifdef CFIO_DEBUG
        open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
        write(1024, *) __FILE__, __LINE__, "[wwj]: write_daary_handle call write_darray" 
        close(1024)
        !print *,__FILE__,__LINE__,"[wwj]: write_daary_handle call write_darray"
#endif
  select case(type)
  case(mpi_integer)
     if(fillv==1) then
...
subroutine readdarray_handler(iosystem)
  ...
  case(mpi_integer)
     !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> zc
     print *, __FILE__, __LINE__, "zc pio debug pio_read_darray"
     !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< zc

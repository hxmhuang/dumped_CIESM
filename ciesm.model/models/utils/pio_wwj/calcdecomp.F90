...
!
  subroutine CalcStartandCount(basetype, ndims, gdims, num_io_procs,myiorank,start, kount, use_io_procs, innermostdecomposed)
    ...
    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> zc
    integer :: num_proc_x, num_proc_y, nth_proc_x, nth_proc_y
    integer :: size_block_x, size_block_y

    num_proc_x = POP_NX_BLOCKS 
    num_proc_y = POP_NY_BLOCKS 
    nth_proc_y = myiorank / num_proc_x
    nth_proc_x = myiorank - nth_proc_y * num_proc_x

    if (ndims > 0) then
        size_block_x = gdims(1) / num_proc_x
        start(1) = size_block_x * nth_proc_x + 1
        kount(1) = size_block_x 
        if (nth_proc_x + 1 == num_proc_x) then
            kount(1) = gdims(1) - size_block_x * (num_proc_x - 1)
        endif 
    endif

    if (ndims > 1) then
        size_block_y = gdims(2) / num_proc_y
        start(2) = size_block_y * nth_proc_y + 1
        kount(2) = size_block_y 
        if (nth_proc_y + 1 == num_proc_y) then
            kount(2) = gdims(2) - size_block_y * (num_proc_y - 1)
        endif 
    endif

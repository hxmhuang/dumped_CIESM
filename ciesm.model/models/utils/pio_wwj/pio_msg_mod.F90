...
!    use pio_types, only : 
    ...
    do while(msg /= -1)
       ...
       case (PIO_MSG_WRITEDARRAY)
        ...
        write(1024, *) __FILE__, __LINE__, "[wwj]: msg_handle call &
        write_darray_handle " 
        close(1024)
        !print *,__FILE__,__LINE__,"[wwj]: msg_handle call write_darray_handle"
#endif
...
!          call mpi_barrier(ios%io_comm,ierr)
       ...
       case default
        ...
        callback_handle" 
        close(1024)
        !print *,__FILE__,__LINE__,"[wwj]: msg_handle call callback_handle"
#endif
       end select   
       if(iorank==0) then
          call mpi_irecv(msg, 1, mpi_integer, ios%comproot, 1, ios%union_comm, req(index), ierr)

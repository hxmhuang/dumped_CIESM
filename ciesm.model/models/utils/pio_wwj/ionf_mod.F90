...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
       ...
       case(PIO_iotype_cfio)
          ...
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_create ... ", fname
          endif
#endif

          !ierr  = nfmpi_create(File%iosystem%IO_comm,fname,nmode ,File%iosystem%info,File%fh)
          ierr = cfio_create(fname, nmode, File%fh)

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_create over ", fname, ierr
          endif
#endif
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
      ...
      if (iotype == PIO_iotype_cfio) then
       ...
       !   write(1024, *) __FILE__, __LINE__, "cfio_mpi_open not defined in cfio! " 
       !   close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_open not defined ... ", fname
          endif
#endif
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_open over ", fname, ierr
          endif
#endif
...
#ifdef CFIO_DEBUG
       !   open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       !   write(1024, *) __FILE__, __LINE__, "cfio_close not defined in cfio! " 
       !   close(1024)
          if (File%iosystem%io_rank == 0) then
             print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_close ..."
          endif
#endif

          !!! matching pnetcdf nf_open, clients try to close the nc
          ierr = nfmpi_close(file%fh) 
          
          !!! matching cfio nf_create, clients arrange server to close the nc
          if (ierr /=  PIO_NOERR) ierr = cfio_close(file%fh)

#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_close over ", ierr
          endif
#endif
#endif
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#ifdef CFIO_DEBUG
       !   open(unit = 1024, status = 'unknown', access = 'append', file = 'cfio_pio_log')
       !   write(1024, *) __FILE__, __LINE__, "cfio_sync not defined in cfio! " 
       !   close(1024)
          if (File%iosystem%io_rank == 0) then
        print *,__FILE__,__LINE__,"[CFIO DEBUG]: cfio_sync not defined"
          endif
#endif
...
#ifdef CFIO_DEBUG
          if (File%iosystem%io_rank == 0) then
            print *,__FILE__,__LINE__,"[CFIO DEBUG]: nfmpi_sync over", ierr
          endif
#endif

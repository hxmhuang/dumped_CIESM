...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> zc
   logical, public :: is_out 
   public :: pio_io_phase_end
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< zc
...
!       endif
    ...
    if(io_comm /= MPI_COMM_NULL) then
            call pio_msg_handler(component_count,iosystem) 
#ifdef CFIO_DEBUG
        print *,__FILE__,__LINE__,"[wwj]: intercomm call msg_handle"
#endif
    endif
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>wwj
    ...
    if(iosystem%num_iotasks.eq.1.and.iotype.eq.pio_iotype_cfio) then	
#if defined(_NETCDF)
       file%iotype=pio_iotype_netcdf
#else
       file%iotype = iotype 
...
!<
  subroutine syncfile(file)
    ...
    case( pio_iotype_cfio)
       ...
       ierr = sync_nf(file)
!#ifdef CFIO_DEBUG
!        print *,__FILE__,__LINE__,"[wwj]: sync_file call &
!        darray_write_comp"
!#endif
    !<<<<<<<<<<<<<<<<<<<<<<<<<<<
    case(pio_iotype_pbinary, pio_iotype_direct_pbinary)
...
!< 
  subroutine closefile(file)
    ...
    case( pio_iotype_cfio)
       ...
       ierr = close_nf(file)
!#ifdef CFIO_DEBUG
!        print *,__FILE__,__LINE__,"[wwj] closefile call darray_write_comp"
!#endif
    !<<<<<<<<<<<<<<<<<<<<<<<<<<<
    case(pio_iotype_binary)
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> zc
  subroutine pio_io_phase_end()
     use cfio
     integer :: ierr

     !print *,__FILE__,__LINE__,"[CESM POP PIO CFIO DEBUG]: cfio_io_end ..."
     ierr = cfio_io_end()
  end subroutine pio_io_phase_end
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< zc

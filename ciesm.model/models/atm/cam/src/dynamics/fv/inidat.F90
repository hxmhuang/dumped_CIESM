...
!
    ...
    if (ideal_phys .or. aqua_planet) then
       ...
       allocate(sgh_tmp(im, jm))
       call infld(fieldname, ncid_topo, 'lon', 'lat', ifirstxy, ilastxy, jfirstxy, jlastxy, &
            dyn_in%sgh, readvar, grid_map='dyn')
       if (.not. readvar) &
            call endrun(trim(subname)//' ERROR: SGHF not found on topo dataset.')
       call process_inidat(ncid_ini, ncid_topo, grid, dyn_in, 'SGH_FLT')
    end if
...
!-----------------
    ...
    case ('SGH_FLT')
      ...
      ! Average SGH_FLT at the poles.
      if ( jfirstxy == 1 ) then
         if (size(sghxy,2) > 0) then
            call par_xsum(grid, sghxy(:,1:1), 1, xsum(1:1) )
            do i=ifirstxy, ilastxy
               sghxy(i,1) = xsum(1) / real(im,r8)
            end do
         end if
      endif
      if ( jlastxy == jm ) then
         call par_xsum(grid, sghxy(:,jm:jm), 1, xsum(1:1) )
         do i=ifirstxy, ilastxy
            sghxy(i,jm) = xsum(1) / real(im,r8)
         enddo
      endif
      do j=jfirstxy,jlastxy
         do i=ifirstxy,ilastxy
            sghxy(i,j) = sghxy(i,j) ** 2
         end do
      end do

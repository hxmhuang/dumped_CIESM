...
!
    ...
    if (jfirstxy==1) then
       ...
       do k = 1, km
       write(iulog,*)"delp max = ", maxval(delpxy(ifirstxy:ilastxy,1,k)), "delp min = ", minval(delpxy(ifirstxy:ilastxy,1,k))
       write(iulog,*)"pt max = ", maxval(ptxy(ifirstxy:ilastxy,1,k)), "pt min = ", minval(ptxy(ifirstxy:ilastxy,1,k))
    ...
    if (jlastxy==jm) then
       ...
       call t_startf("pdadj_reprosum")
       write(iulog,*)"I am before north pole"
       call shr_reprosum_calc(pole, pole_sum, lim, lim, km*(ntotq+2), gbl_count=im, &
                      commid=grid%commxy_x, rel_diff=rel_diff) ! North pole
       write(iulog,*)"I am after north pole"
       call t_stopf("pdadj_reprosum")

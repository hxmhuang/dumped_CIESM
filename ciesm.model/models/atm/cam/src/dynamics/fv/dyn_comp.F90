...
type dyn_import_t
     real(r8), dimension(:,:,:  ), pointer     :: zm     ! altitude of each model layer (unit:m)
     real(r8), dimension(:,: ),    pointer     :: sgh    ! Surface geopotential SGH
...
!-----------------------------------------------------------------------
   ...
   allocate( dyn_in%sgh( I1:IN, J1:JN  ), stat=ierror  )
   if ( ierror /= 0 ) call endrun('DYN_COMP ALLOC error: array SGH')
   allocate( dyn_in%zm( I1:IN, J1:JN, K1:KN  ), stat=ierror  )
   if ( ierror /= 0 ) call endrun('DYN_COMP ALLOC error: array ZM')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine history_defaults
        ...
        !----------------------------------------------------------------------------
        call addfld( 'TAUBBWX' ,'N/m2  ',  1,  'A',  'Zonal      BBW04 wind stress', dyn_decomp )
        call addfld( 'TAUBBWY' ,'N/m2  ',  1,  'A',  'Meridional BBW04 wind stress',  dyn_decomp )
        call add_default( 'TAUBBWX ', 1, ' ' )
        call add_default( 'TAUBBWY ', 1, ' ' )
...
!   use fv_control_mod, only: ct_overlap, trac_decomp
   use fv_control_mod, only: ct_overlap, trac_decomp, oro_drag
...
! Variables from the dynamics interface (import or export)
   ...
   real(r8), pointer :: sghxy(:,:)    ! surface geopotential SGH
   real(r8), pointer :: zmxy(:,:,:)   ! altitude of each model layer 
...
!$omp parallel do private(i,j,k)
   ...
   if (grid%iam .lt. grid%npes_xy) then
!      call orography_drag(ifirstxy, ilastxy, jfirstxy, jlastxy, 1, km, zmxy, sghxy, uxy, vxy, dt )
      call orography_drag(ifirstxy, ilastxy, jfirstxy, jlastxy, 1, km, zmxy, sghxy, uxy, vxy, dt, pexy, t3xy )
   endif

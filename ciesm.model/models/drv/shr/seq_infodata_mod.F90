...
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/release_tags/cesm1_2_x_n01_drvseq4_2_34/shr/seq_infodata_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!EOP
   ...
   type seq_infodata_type
      ...
      logical                 :: albedo_wind     ! T : use wind in albedo, F: old method
...
!         mct_usealltoall, mct_usevector
         mct_usealltoall, mct_usevector, albedo_wind
...
!-------------------------------------------------------------------------------
    ...
    if (seq_comm_iamroot(ID)) then
       ...
       infodata%albedo_wind           = albedo_wind
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   logical             ,optional, intent(OUT) :: albedo_wind   ! T => albedo_wind, false 
...
!-------------------------------------------------------------------------------
    ...
    if ( present(albedo_wind)    ) albedo_wind    = infodata%albedo_wind
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   logical             ,optional, intent(IN) :: albedo_wind    ! T 
...
!-------------------------------------------------------------------------------
    ...
    if ( present(albedo_wind)    ) infodata%albedo_wind    = albedo_wind
...
!-------------------------------------------------------------------------------
    ...
    call shr_mpi_bcast(infodata%albedo_wind,           mpicom)
...
!    if (loglevel > 0) then
     ...
     elseif (trim(infodata%orb_mode) == trim(seq_infodata_orb_variable_year)) then
       ...
       write(logunit,F0L) subname,'albedo_wind              = ', infodata%albedo_wind

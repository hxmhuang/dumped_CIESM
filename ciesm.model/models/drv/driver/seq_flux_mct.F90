...
!--------------------------------------------------------------------------
  ...
  real(r8), allocatable ::  w10  (:)  ! saved w10
...
!===============================================================================
  ...
  subroutine seq_flux_init_mct( cdata, fractions)
    ...
    if(ier/=0) call mct_die(subName,'allocate w10',ier)
    allocate( w10(nloc),stat=ier)
...
!===============================================================================
  ...
  subroutine seq_flux_initexch_mct(cdata_a,cdata_o)
    ...
    if(ier/=0) call mct_die(subName,'allocate duu10n',ier)
    allocate(w10(nloc_a2o),stat=ier)
    if(ier/=0) call mct_die(subName,'allocate w10',ier)
...
!===============================================================================
  ...
  subroutine seq_flux_ocnalb_mct( cdata_o, xao_o, fractions_o)
    ...
    !
   logical  :: albedo_wind
   real(r8) :: p(11)=(/0.0152, -1.7873, 6.8972, -8.5778, 4.071, -7.6446, 0.1643, -7.8409, -3.5639, -2.3588, 10.0538/)
   real(r8) :: n_vis = 1.34
   real(r8) :: n_0 = 1.34
   real(r8) :: n_nir = 1.333
   real(r8) :: chl = 0.02
   !real(r8) :: voldif_vis = -0.00186097868281051 *
   !Chl+0.00088558701039292*log(Chl)+0.00577850291364025*sqrt(Chl)+0.00428440601262417 
   !real(r8) :: voldif_vis = -0.00186097868281051 * 0.02 +
   !0.00088558701039292*log(0.02)+0.00577850291364025*sqrt(0.02)+0.00428440601262417 
   real(r8) :: voldif_vis = 0.0015999534
   real(r8) :: voldif_nir = 0.0
   real(r8) :: w, thetat, theta, segma, f
   real(r8) :: albedoDir0_n0, albedoDir0_vis, albedoDir0_nir, albedoDir_vis, albedoDir_nir, albedoDif_vis, albedoDif_nir
    !-----------------------------------------------------------------------
    ...
    call seq_infodata_GetData(infodata,albedo_wind=albedo_wind)
       ...
       if (nextsw_cday >= -0.5_r8) then
          ...
          do n=1,nloc_o
             ...
             if( albedo_wind ) then
                 theta = acos( cosz )
                 w     = w10(n)
                 segma = sqrt( 0.003 + 0.00512 * w )
                 f     =( p(1) + p(2)*cosz + p(3)*(cosz**2) + p(4)*(cosz**3) + p(5)*segma + p(6)* cosz*segma ) &
                         * exp( p(7) + p(8)*cosz + p(9) *cosz*cosz + p(10)*segma + p(11) *cosz*segma )
                if (cosz  >  0.0_R8) then !--- sun hit --
                    if( theta > 0.001 ) then
                        thetat = asin( sin(theta) / n_0 )
                        albedoDir0_n0 = 0.5*((sin(theta-thetat)/sin(theta+thetat))**2 +(tan(theta-thetat)/tan(theta+thetat))**2)
                        thetat=asin(sin(theta)/n_vis)
                        albedoDir0_vis=0.5*((sin(theta-thetat)/sin(theta+thetat))**2 +(tan(theta-thetat)/tan(theta+thetat))**2)
                        thetat=asin(sin(theta)/n_nir)
                        albedoDir0_nir=0.5*((sin(theta-thetat)/sin(theta+thetat))**2 +(tan(theta-thetat)/tan(theta+thetat))**2)
                    else
                        albedoDir0_n0 = ( ( n_0-1)/ (n_0+1) ) **2
                        albedoDir0_vis=((n_vis-1)/(n_vis+1))**2
                        albedoDir0_nir=((n_nir-1)/(n_nir+1))**2
                    endif
                    if( w > 0.001 ) then
                        albedoDir_vis=albedoDir0_vis-(albedoDir0_vis/albedoDir0_n0)*f + voldif_vis
                        albedoDir_nir=albedoDir0_nir-(albedoDir0_nir/albedoDir0_n0)*f + voldif_nir
                    else
                        albedoDir_vis=albedoDir0_vis + voldif_vis
                        albedoDir_nir=albedoDir0_nir + voldif_nir
                    endif 
                    albedoDif_vis=-0.1482-0.012*segma+0.1608*n_vis-0.0244*n_vis*segma + voldif_vis
                    albedoDif_nir=-0.1482-0.012*segma+0.1608*n_nir-0.0244*n_nir*segma + voldif_nir
                    anidr = albedoDir_nir
                    avsdr = albedoDir_vis
                    anidf = albedoDif_nir
                    avsdf = albedoDif_vis
  ...
  subroutine seq_flux_atmocnexch_mct( cdata_a, cdata_o, a2x, o2x, xao_a, xao_o, fractions_a, fractions_o)
    ...
    if (dead_comps) then
       ...
       do n = 1,nloc_a2o
          ...
          w10(n)  = sqrt( ubot(n) **2 + vbot(n) **2 ) * 0.6
       enddo

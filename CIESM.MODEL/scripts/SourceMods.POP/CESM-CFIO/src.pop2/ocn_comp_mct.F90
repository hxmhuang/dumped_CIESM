...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> zc
       if (is_out) then
          !print *, __FILE__, __LINE__, "[CESM POP PIO CFIO DEBUG] pio_io_phase_end ..."
          call pio_io_phase_end()
          !print *, __FILE__, __LINE__, "[CESM POP PIO CFIO DEBUG] pio_io_phase_end over"
       endif
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< zc

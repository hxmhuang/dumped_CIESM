...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_mld_r03,      &! tavg id for mld r03
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_mld_r03,'mld_r03',2,                         &
                          tavg_method=tavg_method_avg,                &
                          long_name='Mixed-Layer Depth',              &
                          units='centimeter', grid_loc='2110',        &
                          coordinates='TLONG TLAT time')
...
!-----------------------------------------------------------------------
        if (allocated(mld_r03_out)) then
          !$OMP PARALLEL DO PRIVATE(iblock)
          do iblock=1,nblocks_clinic
            call accumulate_tavg_field(mld_r03_out(:,:,iblock), tavg_mld_r03,   iblock, 1)
          end do
          !$OMP END PARALLEL DO
        end if

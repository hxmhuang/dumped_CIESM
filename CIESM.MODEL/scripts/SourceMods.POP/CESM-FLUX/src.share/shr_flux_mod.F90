...
! !EOP
   ...
   real(R8)    :: vmaga  !fhx: surface wind magnitude anomaly (m/s) due to SST 
   real(R8)    :: alpha(nMax)  !fhx: coefficient for wind stress coupling to T diff
...
! fhx:wind stress = Cd*(1+alpha*(To-Ta))*|Uw-Us|*(Uw-Us)
!-------------------------------------------------------------------------------
   ...
   alpha = 0.03  !fhx: alpha need to change spatially!!
   DO n=1,nMax
     if (mask(n) /= 0) then
        ...
        !fhx:beg:wind stress adjustment
         if (ts(n)>280) then
         vmaga=alpha(n)*(-1)*delt*vmag
         else
         vmaga=0
         endif
        ! write(s_logunit,*) 'ts(n)=',ts(n) 
        ! write(s_logunit,*) 'bf vmag=',vmag,'ts(n)=',ts(n) 
         vmag = max(umin, vmag+vmaga) 
        ! write(s_logunit,*) 'aft vmag=',vmag,'vmaga=',vmaga 
        !fhx:end:wind stress adjustment
        !------------------------------------------------------------

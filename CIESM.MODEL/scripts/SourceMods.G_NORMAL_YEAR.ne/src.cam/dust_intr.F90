  ...
  subroutine Dustini()
    ...
    if (dust_number == 4) then
       ...
       do n = 1, dust_number
          ...
          print *, "dingnan dust_number ",  dust_number
          print *, "dingnan dmt_min ", dmt_min(n)
          print *, "dingnan dmt_max ", dmt_max(n)
          print *, "dingnan dmt_ctr ", dmt_ctr(n) 
          print *, "dingnan dmt_dlt ", dmt_dlt(n) 
       end do
    else
#if (defined MODAL_AERO)
    ...
    do n = 1, dust_number
       series_ratio = (dmt_max(n)/dmt_min(n))**(1.0_r8/sz_nbr)
          print *, "dingnan sz_nbr ",  sz_nbr
          print *, "dingnan series_ratio  ",  series_ratio
       sz_min(1) = dmt_min(n)
      ...
      ! write(*,*) "dingnan---sz_ctr(m): ",sz_ctr(m)
      ! write(*,*) "dingnan---dmt_nma: ",dmt_nma
      ! write(*,*) "dingnan---ln_gsd: ",ln_gsd
      ! write(*,*) "dingnan---ndst: ",ndst
      ! write(*,*) "dingnan----tmp: ",tmp
      ! write(*,*) "dingnan----lgn_dst: ",lgn_dst
      ...
      ! write(*,*) "dingnan----dmt_vwr: ",dmt_vwr(n)
      ! write(*,*) "dingnan----vlm_rsl: ",vlm_rsl(n)
    ...
    do m = 1, dust_number
       ...
       do while(eps_crr > eps_max)
          ...
          write(*,*) "dingnan---ryn_nbr_grv(m): " ,ryn_nbr_grv(m)
          write(*,*) "dingnan---vlc_grv(m): " ,vlc_grv(m)
          write(*,*) "dingnan---dmt_vwr(m): " ,dmt_vwr(m) 
          write(*,*) "dingnan---vsc_knm_atm: " ,vsc_knm_atm 
  ...
  subroutine DustDryDep(ncol,t,pmid,ram1,fv,vlc_dry,vlc_trb,vlc_grv,landfrac)
    ...
    do k=1,pver
       do i=1,ncol
          ...
          write(*,*)"dingnan ddd--- vsc_dyn_atm(i,k) ", vsc_dyn_atm(i,k) 

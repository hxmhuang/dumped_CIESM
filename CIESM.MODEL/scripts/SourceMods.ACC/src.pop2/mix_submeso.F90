...
!-----------------------------------------------------------------------
   real (r8) :: &
      sqrt_grav              ! sqrt(grav)
   !integer (int_kind) :: &
!      timer_submeso_sf_ori,& 
      !timer_submeso_sf_acc
!EOC
!***********************************************************************
...
!  call get_timer(timer_submeso_sf_ori,'SUBMESO_SF_ORI', &
!                                  nblocks_clinic, distrb_clinic%nprocs)
  !call get_timer(timer_submeso_sf_acc,'SUBMESO_SF_ACC', &
  !                                nblocks_clinic, distrb_clinic%nprocs)
!-----------------------------------------------------------------------
!EOC
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block,2,2,km)  :: &
      acc_SF_SUBM_X,  &       ! components of the submesoscale 
      acc_SF_SUBM_Y           !  streamfunction
   real (r8), dimension(nx_block,ny_block) :: &
      acc_ML_DEPTH,               &  ! horizontal length scale
      acc_HLS        

   real (r8) :: &
      zw_top, factor

   real (r8) :: err1, err2, err3
...
!  call timer_start(timer_submeso_sf_ori,block_id=bid)
...
!   call timer_stop(timer_submeso_sf_ori,block_id=bid)
    
   !call timer_start(timer_submeso_sf_acc)
   ...
   !call timer_stop(timer_submeso_sf_acc)

!   err3 = maxval(abs(acc_SF_SUBM_X - SF_SUBM_X(:,:,:,:,:,bid)))
!   err2 = maxval(abs(SF_SUBM_X(:,:,:,:,:,bid)))
!   err1 = maxval(abs(acc_SF_SUBM_X))
!
!!   if (my_task == master_task) then
!    if (err3/(err2+1.0e-10) > 1.0e-14) then
!       write(*,*) "submeso_sf_acc X", err2," - ",  err1, " = ", err3
!    endif
!!   endif 
...
!***********************************************************************
   ...
   real (8), dimension(km),intent(in) :: zw, dz, zt
   integer (4), dimension(nx_block,ny_block), intent(in):: KMT
   real (8), dimension(nx_block,ny_block), intent(in):: HMXL
   real (8), dimension(nx_block,ny_block), intent(in):: DXT, DYT
   real (8), dimension(0:km),intent(in) :: dzw, dzwr
   real (8), dimension(nx_block,ny_block,km), intent(in) :: RZ_SAVE
   real (8), dimension(nx_block,ny_block,2,km), intent(in) :: RX, RY
   real (8), dimension(nx_block,ny_block), intent(in) :: TIME_SCALE
   real (8), intent(in) :: &
      efficiency_factor, &
      max_hor_grid_scale, &   
      grav , &   
   ...
   real (8), dimension(nx_block,ny_block), intent(inout) :: & 
      ML_DEPTH
   real (8), dimension(nx_block,ny_block,2,2,km),intent(inout) :: &
      SF_SUBM_X, SF_SUBM_Y

   real (8), dimension(2,km,nx_block,ny_block)  :: ts_RX, ts_RY
   real (8), dimension(2,2,km,nx_block,ny_block):: ts_SF_SUBM_X, ts_SF_SUBM_Y

   integer (4) :: k, i,j ,kk
   logical (4) :: CONTINUE_INTEGRAL
   real (8)  :: BX_VERT_AVG1, BY_VERT_AVG1, BX_VERT_AVG2, BY_VERT_AVG2           
   real (8)  :: WORK1, WORK2, WORK3, ML_DEPTH_acc, TIME_SCALE_acc
   real (8)  :: DXT_acc,DYT_acc, HLS_acc
   integer (4)  :: KMT_acc
   real (8)  :: zw_top
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !$acc parallel loop &
   !$acc copyin(RX,RY) &
   !$acc copyout(ts_RX,ts_RY)
   do k=1,km
     do j=1,ny_block
       do i=1,nx_block
          ts_RX(:,k,i,j) = RX(i,j,:,k)    
          ts_RY(:,k,i,j) = RY(i,j,:,k)    
   ...
   !$acc swapin(RZ_SAVE(dimension order : 3,1,2)) &
   !$acc copyin(zw,dz,zt,dzw,dzwr,KMT,HMXL,DXT,DYT,TIME_SCALE,ts_RX,ts_RY) &
   !$acc copyout(HLS,ts_SF_SUBM_X,ts_SF_SUBM_Y,ML_DEPTH) &
   !$acc annotate(entire(zw,dz,zt,dzw,dzwr)) 
   ...
   ML_DEPTH_acc = HMXL(i,j) 
   ML_DEPTH(i,j) = ML_DEPTH_acc

   TIME_SCALE_acc = TIME_SCALE(i,j)
   KMT_acc = KMT(i,j)
   DXT_acc = DXT(i,j)
   DYT_acc = DYT(i,j)
   HLS_acc  = c0
...
!-----------------------------------------------------------------------
   ...
   do k=1,km
     ...
     if ( k > 1 )  zw_top = zw(k-1)
     WORK3 = c0
     if ( CONTINUE_INTEGRAL  .and.  ML_DEPTH_acc > zw(k) ) then
        WORK3 = dz(k)
     endif
     if ( CONTINUE_INTEGRAL  .and.  ML_DEPTH_acc <= zw(k)  &
             .and.  ML_DEPTH_acc > zw_top ) then
       WORK3 = ML_DEPTH_acc - zw_top
     endif

     if ( CONTINUE_INTEGRAL ) then
       !BX_VERT_AVG1 = BX_VERT_AVG1        &
       !                    + RX(i,j,1,k) * WORK3
       !BX_VERT_AVG2 = BX_VERT_AVG2        &
       !                    + RX(i,j,2,k) * WORK3
       !BY_VERT_AVG1 = BY_VERT_AVG1        &
       !                    + RY(i,j,1,k) * WORK3
       !BY_VERT_AVG2 = BY_VERT_AVG2        &
       !                    + RY(i,j,2,k) * WORK3
       BX_VERT_AVG1 = BX_VERT_AVG1        &
                           + ts_RX(1,k,i,j) * WORK3
       BX_VERT_AVG2 = BX_VERT_AVG2        &
   ...
   if ( KMT_acc > 0 ) then
     BX_VERT_AVG1 = - grav * BX_VERT_AVG1 / ML_DEPTH_acc
     BX_VERT_AVG2 = - grav * BX_VERT_AVG2 / ML_DEPTH_acc
     BY_VERT_AVG1 = - grav * BY_VERT_AVG1 / ML_DEPTH_acc
     BY_VERT_AVG2 = - grav * BY_VERT_AVG2 / ML_DEPTH_acc
   endif
...
!-----------------------------------------------------------------------
       ...
       if ( KMT_acc > 0 ) then
         WORK1 = sqrt( p5 * (                                       &
         ( BX_VERT_AVG1**2 + BX_VERT_AVG2**2 )  &
         / DXT_acc**2                                &
         + ( BY_VERT_AVG1**2 + BY_VERT_AVG2**2 )  &
         / DYT_acc**2 ) )
         WORK1 = WORK1 * ML_DEPTH_acc * (TIME_SCALE_acc**2)
       endif
   ...
   do k=2,km
     ...
     WORK3 = c0
     if ( CONTINUE_INTEGRAL  .and.  ML_DEPTH_acc > zt(k) ) then
       WORK3 = dzw(k-1) 
     endif
     if ( CONTINUE_INTEGRAL  .and.  ML_DEPTH_acc <= zt(k)  &
          .and.  ML_DEPTH_acc >= zt(k-1) ) then
       WORK3 = ( (ML_DEPTH_acc - zt(k-1))**2 ) * dzwr(k-1)
     endif

     if ( CONTINUE_INTEGRAL ) then
       WORK2 = WORK2 + sqrt(-RZ_SAVE(i,j,k) * WORK3)
     endif
   ...
   enddo
       ...
       if ( KMT_acc > 0 ) then
         ...
         WORK2 = sqrt_grav * WORK2 * TIME_SCALE_acc

         HLS_acc = max ( WORK1, WORK2, hor_length_scale )
...
!-----------------------------------------------------------------------
   ...
   do k=1,km
     ...
     do kk=ktp,kbt
         if ( reference_depth(kk) < ML_DEPTH_acc  .and.  &
            KMT_acc >= k ) then

         WORK3 = ( c1 - ( c2 * reference_depth(kk) / ML_DEPTH_acc ) )**2
            
         WORK2 = ( c1 - WORK3 )  &
               * ( c1 + ( 5.0_8 / 21.0_8 ) * WORK3 )

         WORK1 = efficiency_factor * (ML_DEPTH_acc**2) * WORK2  &
                * TIME_SCALE_acc / HLS_acc


         !SF_SUBM_X(i,j,1,kk,k) = WORK1 * BX_VERT_AVG1  &
         !                           * min(DXT_acc,max_hor_grid_scale)
         !SF_SUBM_X(i,j,2,kk,k) = WORK1 * BX_VERT_AVG2  &
         !                           * min(DXT_acc,max_hor_grid_scale)
         !SF_SUBM_Y(i,j,1,kk,k) = WORK1 * BY_VERT_AVG1  &
         !                           * min(DYT_acc,max_hor_grid_scale)
         !SF_SUBM_Y(i,j,2,kk,k) = WORK1 * BY_VERT_AVG2  &
         !                           * min(DYT_acc,max_hor_grid_scale)
         ts_SF_SUBM_X(1,kk,k,i,j) = WORK1 * BX_VERT_AVG1  &
                                   * min(DXT_acc,max_hor_grid_scale)
         ts_SF_SUBM_X(2,kk,k,i,j) = WORK1 * BX_VERT_AVG2  &
                                   * min(DXT_acc,max_hor_grid_scale)
         ts_SF_SUBM_Y(1,kk,k,i,j) = WORK1 * BY_VERT_AVG1  &
                                   * min(DYT_acc,max_hor_grid_scale)
         ts_SF_SUBM_Y(2,kk,k,i,j) = WORK1 * BY_VERT_AVG2  &
                                    * min(DYT_acc,max_hor_grid_scale)
   ...
   !$acc parallel loop &
   !$acc copyin(ts_SF_SUBM_X) &
   !$acc copyout(SF_SUBM_X) 
   do k=1,km
     do j=1,ny_block
       do i=1,nx_block
          SF_SUBM_X(i,j,:,:,k) = ts_SF_SUBM_X(:,:,k,i,j)    
       enddo
     enddo
   enddo 
   !$acc end parallel loop
   ...
   !$acc parallel loop &
   !$acc copyin(ts_SF_SUBM_Y) &
   !$acc copyout(SF_SUBM_Y) 
   do k=1,km
     do j=1,ny_block
       do i=1,nx_block
          SF_SUBM_Y(i,j,:,:,k) = ts_SF_SUBM_Y(:,:,k,i,j)    
       enddo
     enddo
   enddo 
   !$acc end parallel loop
   
   end subroutine submeso_sf_acc

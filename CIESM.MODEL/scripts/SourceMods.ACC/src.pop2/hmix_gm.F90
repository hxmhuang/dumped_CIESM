...
!-----------------------------------------------------------------------
      ...
      !real (r8), dimension(:,:,:,:,:,:), allocatable :: &
      !   tmp_SF_SLX, tmp_SF_SLY       ! components of the merged streamfunction
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      !timer_gm_merged_ori,&         ! main n loop
      !timer_gm_merged_acc,&         ! main n loop
      !timer_transition_acc
...
!-----------------------------------------------------------------------
    ...
    !allocate (tmp_SF_SLX(nx_block,ny_block,2,2,km,nblocks_clinic),  &
    !         tmp_SF_SLY(nx_block,ny_block,2,2,km,nblocks_clinic))
...
!-----------------------------------------------------------------------
   ...
   if ( diag_gm_bolus ) then
     ...
     call define_tavg_field (tavg_VNS_ISOP, 'VNS_ISOP', 3,                               &
      ...
      !call get_timer(timer_gm_merged_ori,'HMIX_TRACER_GM_MERGED_ORI', &
      !                            nblocks_clinic, distrb_clinic%nprocs)
      !call get_timer(timer_gm_merged_acc,'HMIX_TRACER_GM_MERGED_ACC', &
      !                            nblocks_clinic, distrb_clinic%nprocs)
      !call get_timer(timer_transition_acc,'TRANSITION_ACC', &
      !                            nblocks_clinic, distrb_clinic%nprocs)
!-----------------------------------------------------------------------
!EOC
...
!-----------------------------------------------------------------------
      ...
      if ( k == 1 ) then
        ...
        if ( transition_layer_on ) then
          ...
          !call timer_start(timer_transition_acc,block_id = bid)
          call my_transition_layer(          &
          nx_block,                     &
          ny_block,                     &
          km,                           &
          KMT,                          & 
          ...
          TLT%THICKNESS(:,:,bid),       &
          TLT%INTERIOR_DEPTH(:,:,bid),  &            
          TLT%K_LEVEL(:,:,bid),         &
          TLT%ZTW(:,:,bid)              &
          )
          !call timer_stop(timer_transition_acc,block_id = bid)
        endif
...
!     B.C. at the top
        ...
        if ( transition_layer_on ) then
          ...
          !call timer_start(timer_gm_merged_ori, block_id=this_block%local_id)
          !call merged_streamfunction ( this_block )
          !call timer_stop(timer_gm_merged_ori, block_id=this_block%local_id)

          !call timer_start(timer_gm_merged_acc, block_id=this_block%local_id)
          !------------------------------------------------------------------
          !edit by nyf, call the merged_streamfunction with openacc
          !------------------------------------------------------------------
          call my_merged_streamfunction &
           (nx_block, &
            ny_block, &
            km,&
            TLT%K_LEVEL(:,:,bid), &
            TLT%ZTW(:,:,bid) ,       &  
            TLT%DIABATIC_DEPTH(:,:,bid),  &   
            TLT%THICKNESS(:,:,bid),       &   ! transition layer thickness
            TLT%INTERIOR_DEPTH(:,:,bid)  ,& 
            KMT(:,:,bid)      ,& 
            KAPPA_THIC(:,:,:,:,bid) ,&
            SF_SLX(:,:,:,:,:,bid), &
            ...
            dzwr,        & 
            dz    )
          !call timer_stop(timer_gm_merged_acc, block_id=this_block%local_id)
          !!------------------------------------------------------------------
          !
          !!call timer_stop(timer_gm_merged_acc, block_id=this_block%local_id)
          !if(my_task .eq. master_task) then
          !  print *,my_task,sum(tmp_SF_SLX-SF_SLX) + sum(tmp_SF_SLY-SF_SLY)
          !endif
...
!EOC
      ...
      subroutine my_merged_streamfunction &
        (my_nx_block, &
         my_ny_block, &
         my_km,&
         my_K_LEVEL, &
         my_ZTW ,       &  
         my_DIABATIC_DEPTH,  &   ! depth of the diabatic region at the
         my_THICKNESS,       &   ! transition layer thickness
         my_INTERIOR_DEPTH  ,& 
         my_KMT      ,& 
         my_KAPPA_THIC ,&
         my_SF_SLX, &
...
!-----------------------------------------------------------------------
      integer (4), intent(in) :: & 
         my_nx_block, my_ny_block, my_km

      integer (4), dimension(my_nx_block,my_ny_block) , intent(in)  :: &
         my_K_LEVEL,  &          ! k level at or below which the interior,
         my_ZTW       , &          ! designates if the interior region
         my_KMT       

      real (8), dimension(my_nx_block,my_ny_block), intent(in) :: &
         my_DIABATIC_DEPTH,  &   ! depth of the diabatic region at the
         my_THICKNESS,       &   ! transition layer thickness
         my_INTERIOR_DEPTH       ! depth at which the interior, adiabatic

      real (8), dimension(my_nx_block,my_ny_block,2,my_km), intent(in) :: &
        my_KAPPA_THIC
      real (8), dimension(0:my_km), intent(in):: &
        my_dzwr          ! midpoint of k to midpoint of k+1
      real (8), dimension(my_km), intent(in):: &
        my_dz          ! midpoint of k to midpoint of k+1

      real (8), dimension(my_nx_block,my_ny_block,2,2,my_km),intent(inout) :: &
        my_SF_SLX, my_SF_SLY

      real (8), dimension(my_nx_block,my_ny_block,2,2,my_km),intent(in) :: &
        my_SLX, my_SLY
      
      real (8), dimension(2,2,my_km,my_nx_block,my_ny_block) :: &
        ts_SLX,ts_SLY
      
      real (8), dimension(2,2,my_km,my_nx_block,my_ny_block) :: &
        ts_SF_SLX, ts_SF_SLY
      
      real (8), dimension(2,my_km,my_nx_block,my_ny_block) :: &
        ts_KAPPA_THIC
      ...
      real (8) :: &
         WORK5, WORK6          ! more work arrays

      logical (log_kind) :: &
         LMASK               ! flag
      
      real (8) ::  maxerr1,maxerr2
      real (8), dimension(2) :: &
         reference_depth              ! zt or zw
...
!-----------------------------------------------------------------------
    ...
    !$acc parallel loop & 
    !$acc copyin(my_KAPPA_THIC,my_SLY) &
    !$acc copyout(ts_KAPPA_THIC,ts_SLY)
    do k=1,km 
      do j=1,ny_block
        do i=1,nx_block
          ts_KAPPA_THIC(:,k,i,j) = my_KAPPA_THIC(i,j,:,k)
          ts_SLY(:,:,k,i,j) = my_SLY(i,j,:,:,k)
        enddo
      enddo
    enddo
    ...
    !$acc copyin(zt,my_dz,my_dzwr,my_DIABATIC_DEPTH,my_THICKNESS,my_KMT,my_INTERIOR_DEPTH,my_ZTW,my_K_LEVEL,ts_KAPPA_THIC,ts_SLX,ts_SLY) &
    !$acc copyout(ts_SF_SLX,ts_SF_SLY) &
    !$acc annotate(entire(zt,my_dz,my_dzwr)) &
    !$acc local(LMASK,WORK1,WORK2,WORK3,WORK4,WORK5,WORK6,WORK2_NEXT,WORK4_NEXT,reference_depth)
    do j=1,ny_block
      do i=1,nx_block
        WORK1 = c0
        WORK2 = c0
        ...
        do k=1,km-1
          do kk=1,2
              LMASK = ((my_K_LEVEL(i,j) == k)  .and. (my_K_LEVEL(i,j) < my_KMT(i,j))  .and.  (my_ZTW(i,j) == 1))
              if(LMASK) then
                WORK1(kk) =  ts_KAPPA_THIC(kbt,k,i,j) * ts_SLX(kk,kbt,k,i,j) * my_dz(k)
                WORK2(kk) = c2 * my_dzwr(k) * ( WORK1(kk) - ts_KAPPA_THIC(ktp,k+1,i,j) * ts_SLX(kk,ktp,k+1,i,j) * my_dz(k+1) )
                WORK2_NEXT = c2 * ( ts_KAPPA_THIC(ktp,k+1,i,j) * ts_SLX(kk,ktp,k+1,i,j) - ts_KAPPA_THIC(kbt,k+1,i,j) * ts_SLX(kk,kbt,k+1,i,j) )
                
                WORK3(kk) =  ts_KAPPA_THIC(kbt,k,i,j) * ts_SLY(kk,kbt,k,i,j) * my_dz(k)
                WORK4(kk) = c2 * my_dzwr(k) * ( WORK3(kk) - ts_KAPPA_THIC(ktp,k+1,i,j) * ts_SLY(kk,ktp,k+1,i,j) * my_dz(k+1) )
                WORK4_NEXT = c2 * ( ts_KAPPA_THIC(ktp,k+1,i,j) * ts_SLY(kk,ktp,k+1,i,j) - ts_KAPPA_THIC(kbt,k+1,i,j) * ts_SLY(kk,kbt,k+1,i,j) )
                 
                if (abs(WORK2_NEXT) < abs(WORK2(kk))) WORK2(kk) = WORK2_NEXT 
                if (abs(WORK4_NEXT) < abs(WORK4(kk))) WORK4(kk) = WORK4_NEXT
               endif

               LMASK = ((my_K_LEVEL(i,j) == k) .and. (my_K_LEVEL(i,j) < my_KMT(i,j))  .and. (my_ZTW(i,j) == 2))
               if(LMASK) then
                    WORK1(kk) =  ts_KAPPA_THIC(ktp,k+1,i,j)     & 
                    * ts_SLX(kk,ktp,k+1,i,j)
                    WORK2(kk) =  c2 * ( WORK1(kk)                 &
                    - ( ts_KAPPA_THIC(kbt,k+1,i,j)        &
                    * ts_SLX(kk,kbt,k+1,i,j) ) )
                    WORK1(kk) = WORK1(kk) * my_dz(k+1)

                    WORK3(kk) =  ts_KAPPA_THIC(ktp,k+1,i,j)     &
                    * ts_SLY(kk,ktp,k+1,i,j)
                    WORK4(kk) =  c2 * ( WORK3(kk)                 &
                    - ( ts_KAPPA_THIC(kbt,k+1,i,j)        &
                    * ts_SLY(kk,kbt,k+1,i,j) ) )
                    WORK3(kk) = WORK3(kk) * my_dz(k+1)
                endif
                
                LMASK = (LMASK .and. (my_K_LEVEL(i,j) + 1 < my_KMT(i,j)))
                if(LMASK) then
                  WORK2_NEXT = c2 * my_dzwr(k+1) * ( &
                  ts_KAPPA_THIC(kbt,k+1,i,j) * ts_SLX(kk,kbt,k+1,i,j) * my_dz(k+1) - &
                  ts_KAPPA_THIC(ktp,k+2,i,j) * ts_SLX(kk,ktp,k+2,i,j) * my_dz(k+2))
                  WORK4_NEXT = c2 * my_dzwr(k+1) * ( &
                  ts_KAPPA_THIC(kbt,k+1,i,j) * ts_SLY(kk,kbt,k+1,i,j) * my_dz(k+1) - &
                  ts_KAPPA_THIC(ktp,k+2,i,j) * ts_SLY(kk,ktp,k+2,i,j) * my_dz(k+2))
                  if(abs(WORK2_NEXT) < abs(WORK2(kk)) ) WORK2(kk) = WORK2_NEXT
                  if(abs(WORK4_NEXT) < abs(WORK4(kk)) ) WORK4(kk) = WORK4_NEXT
                endif
              enddo
            ...
            do k=1,km
              do kk=ktp,kbt
                ts_SF_SLX(1,kk,k,i,j) =c0
                ts_SF_SLX(2,kk,k,i,j) =c0
                ts_SF_SLY(1,kk,k,i,j) =c0
                ts_SF_SLY(2,kk,k,i,j) =c0
                if(k<=my_KMT(i,j)) then

                  if (my_KMT(i,j) /= 0) then 
                    WORK5 = c1 / ( c2 * my_DIABATIC_DEPTH(i,j) + my_THICKNESS(i,j) )
                  else 
                    WORK5 = c0
                  endif

                  if((my_KMT(i,j) /= 0) .AND. (my_THICKNESS(i,j) > eps)) then 
                    WORK6 = WORK5 / my_THICKNESS(i,j)
                  else
                    WORK6 = c0
                  endif

                  reference_depth(ktp) = zt(k) - p25 * my_dz(k)
                  reference_depth(kbt) = zt(k) + p25 * my_dz(k)

                  if(reference_depth(kk)<=my_DIABATIC_DEPTH(i,j)) then
                    ts_SF_SLX(1,kk,k,i,j) = reference_depth(kk) * WORK5  &
                    * ( c2 * WORK1(1) + my_THICKNESS(i,j)  &
                    * WORK2(1) )

                    ts_SF_SLX(2,kk,k,i,j) = reference_depth(kk) * WORK5  &
                    * ( c2 * WORK1(2) + my_THICKNESS(i,j)  &
                    * WORK2(2) )

                    ts_SF_SLY(1,kk,k,i,j) = reference_depth(kk) * WORK5  &
                    * ( c2 * WORK3(1) + my_THICKNESS(i,j)  &
                    * WORK4(1) )

                    ts_SF_SLY(2,kk,k,i,j) = reference_depth(kk) * WORK5  &
                    * ( c2 * WORK3(2) + my_THICKNESS(i,j)  &
                    * WORK4(2) )

                  else if( reference_depth(kk) > my_DIABATIC_DEPTH(i,j) .and.  reference_depth(kk) <= my_INTERIOR_DEPTH(i,j)) then

                    ts_SF_SLX(1,kk,k,i,j) = - (my_DIABATIC_DEPTH(i,j) - reference_depth(kk))**2 * WORK6            &
                    * ( WORK1(1) + my_INTERIOR_DEPTH(i,j)  &
                    * WORK2(1) )                             &
                    + reference_depth(kk) * WORK5                &
                    * ( c2 * WORK1(1) + my_THICKNESS(i,j)  &
                    * WORK2(1) )

                    ts_SF_SLX(2,kk,k,i,j) = - (my_DIABATIC_DEPTH(i,j) - reference_depth(kk))**2 * WORK6             &
                    * ( WORK1(2) + my_INTERIOR_DEPTH(i,j)  &
                    * WORK2(2) )                             &
                    + reference_depth(kk) * WORK5                    &
                    * ( c2 * WORK1(2) + my_THICKNESS(i,j)  &
                    * WORK2(2) )

                    ts_SF_SLY(1,kk,k,i,j) = - (my_DIABATIC_DEPTH(i,j) - reference_depth(kk))**2 * WORK6              &
                    * ( WORK3(1) + my_INTERIOR_DEPTH(i,j)  &
                    * WORK4(1) )                             &
                    + reference_depth(kk) * WORK5                    &
                    * ( c2 * WORK3(1) + my_THICKNESS(i,j)  &
                    * WORK4(1) )

                    ts_SF_SLY(2,kk,k,i,j) = - (my_DIABATIC_DEPTH(i,j) - reference_depth(kk))**2 * WORK6            &
                    * ( WORK3(2) + my_INTERIOR_DEPTH(i,j)  &
                    * WORK4(2) )                             &
                    + reference_depth(kk) * WORK5                    &
                    * ( c2 * WORK3(2) + my_THICKNESS(i,j)  &
                    * WORK4(2) )

                  else if( reference_depth(kk) > my_INTERIOR_DEPTH(i,j)) then
                    ts_SF_SLX(1,kk,k,i,j) =  ts_KAPPA_THIC(kk,k,i,j) * ts_SLX(1,kk,k,i,j) * my_dz(k)
                                       
                    ts_SF_SLX(2,kk,k,i,j) =  ts_KAPPA_THIC(kk,k,i,j) * ts_SLX(2,kk,k,i,j) * my_dz(k)
                                       
                    ts_SF_SLY(1,kk,k,i,j) =  ts_KAPPA_THIC(kk,k,i,j) * ts_SLY(1,kk,k,i,j) * my_dz(k)
                                       
                    ts_SF_SLY(2,kk,k,i,j) =  ts_KAPPA_THIC(kk,k,i,j) * ts_SLY(2,kk,k,i,j) * my_dz(k)
         ...
         !$acc parallel loop & 
         !$acc copyin(ts_SF_SLX) &
         !$acc copyout(my_SF_SLX)
         do k=1,km 
           do j=1,ny_block
             do i=1,nx_block
               my_SF_SLX(i,j,:,:,k) = ts_SF_SLX(:,:,k,i,j)
             enddo
           enddo
         enddo
         !$acc end parallel loop     
         ...
         !$acc parallel loop & 
         !$acc copyin(ts_SF_SLY) &
         !$acc copyout(my_SF_SLY)
         do k=1,km 
           do j=1,ny_block
             do i=1,nx_block
               my_SF_SLY(i,j,:,:,k) = ts_SF_SLY(:,:,k,i,j)
             enddo
           enddo
         enddo
         !$acc end parallel loop     
         ...
         !!$acc parallel loop &
         !!$acc copyin(zt,my_dz,my_DIABATIC_DEPTH, my_THICKNESS,WORK1,WORK2,WORK3 ,WORK4, WORK5,WORK6,my_KMT,my_INTERIOR_DEPTH)  &
         !!$acc copyout(my_SF_SLX,my_SF_SLY) &
         !!!$acc annotate(entire(my_KMT,my_DIABATIC_DEPTH,my_INTERIOR_DEPTH,my_THICKNESS,WORK1,WORK2,WORK3,WORK4,WORK5,WORK6,zt,my_dz))&
         !!$acc local(reference_depth) 
         !do k=1,km
         !  do kk=ktp,kbt
         !    do j=1,ny_block
         !      do i=1,nx_block
    ...
    !! check consistency
!    if(my_task .eq. master_task) then
!      maxerr2 = maxval(abs(tmp_my_SF_SLX))
!      maxerr1 = maxval(abs(tmp_my_SF_SLX - my_SF_SLX))
!      write(*,*) "hmix_gm_merged_maxval_x",my_task, maxerr1, maxerr2
!      maxerr1 = maxval(abs(tmp_my_SF_SLY - my_SF_SLY))
!      write(*,*) "hmix_gm_merged_maxval_Y",my_task, maxerr1,maxerr2
!    endif

!-----------------------------------------------------------------------
!EOC

      end subroutine my_merged_streamfunction
...
! my_transition_layer
!***************************************************************************
    subroutine my_transition_layer( &
               nx_block,            &
               ny_block,            &
               km,                  &
               KMT,                 & 
...
!----------------------------------------------------------------------------
      implicit none
      integer (4), intent(in) :: nx_block, ny_block, km
      integer(4), dimension(nx_block, ny_block), intent(in) :: KMT 
      real (8), dimension(km), intent(in) :: zt, zw
      real (8), dimension(nx_block, ny_block, 2, km), intent(in) :: SLA_SAVE
      real (8), dimension(nx_block, ny_block), intent(in):: RB
      real (8), dimension(nx_block,ny_block), intent(in) :: DIABATIC_DEPTH   



      real (8), dimension(nx_block,ny_block), intent(inout) :: &
            THICKNESS,     &
            INTERIOR_DEPTH            
      integer (4), dimension(nx_block,ny_block), intent(inout):: &
            K_LEVEL,       &
            ZTW


      real (8) :: WORK     
      real (8), dimension(2) :: reference_depth  
      integer (4) :: K_START, K_SUB
      logical (4) :: COMPUTE_TLT
      integer (4) :: i, j, k, kk 
      integer (4), parameter :: ktp = 1, kbt = 2 
      real (8),parameter :: c0 = 0.0_8

   !$acc parallel loop &
   !$acc copyin(KMT,zw,zt,SLA_SAVE,RB,DIABATIC_DEPTH) &
   !$acc copy(THICKNESS,INTERIOR_DEPTH,K_LEVEL,ZTW) &
   !$acc local(reference_depth)&
   !$acc annotate(entire(zt,zw))

      do j = 1, ny_block
      do i = 1, nx_block

        THICKNESS(i,j)      = c0
        INTERIOR_DEPTH(i,j) = c0
        K_LEVEL(i,j)        = 0
        ZTW(i,j)            = 0
        K_START = 0
        K_SUB = 0
      ...
      do k=1,km
        if ( COMPUTE_TLT  .and.  &
                DIABATIC_DEPTH(i,j) < zw(k) ) then

          K_START = k+1
          K_SUB   = ktp

          THICKNESS(i,j) = zw(k) - DIABATIC_DEPTH(i,j)
          K_LEVEL(i,j)   = k
          ZTW(i,j)       = 2
        ...
        if ( k /= 1  .and.  K_START == k+1  .and. &
                DIABATIC_DEPTH(i,j) < zt(k) ) then

          K_START = k
          K_SUB   = kbt

          THICKNESS(i,j) = zt(k) - DIABATIC_DEPTH(i,j)
          K_LEVEL(i,j)   = k
          ZTW(i,j)       = 1
      ...
      if ( KMT(i,j) == 0  .or.  K_START > KMT(i,j)  .or.  &
         ( K_START == KMT(i,j)  .and.  K_SUB == kbt ) ) then
        COMPUTE_TLT = .false.
      else
        COMPUTE_TLT = .true.
      endif
      ...
      do k=1,km-1
        ...
        if ( COMPUTE_TLT  .and.  K_SUB == kbt  .and.  &
                K_START < KMT(i,j)  .and.  K_START == k ) then
          WORK = max(SLA_SAVE(i,j,kbt,k), &
                     SLA_SAVE(i,j,ktp,k+1)) * RB(i,j)
        endif

        if ( WORK /= c0  .and.  &
                DIABATIC_DEPTH(i,j) <  (zw(k) - WORK) ) then
          COMPUTE_TLT = .false.
        endif
      ...
      do k=2,km
        ...
        do kk=ktp,kbt
          ...
          if (kk == ktp) then
            if( COMPUTE_TLT  .and.  K_START <= KMT(i,j)  .and. &
                    K_START == k ) then
              WORK = max(SLA_SAVE(i,j,ktp,k), &
                         SLA_SAVE(i,j,kbt,k)) * RB(i,j)
            endif
          else
            if ( COMPUTE_TLT  .and.  K_START < KMT(i,j)  .and. &
                    K_START == k ) then
            ...
            if ( COMPUTE_TLT  .and.  K_START == KMT(i,j)  .and. &
                    K_START == k ) then
              WORK = SLA_SAVE(i,j,kbt,k) * RB(i,j)
            endif
          endif

          if ( WORK /= c0  .and.  &
                  DIABATIC_DEPTH(i,j) <  (reference_depth(kk) - WORK)) then
            COMPUTE_TLT = .false.
          endif

          if ( WORK /= c0  .and.  &
                  DIABATIC_DEPTH(i,j) >= (reference_depth(kk) - WORK)) then
            THICKNESS(i,j) = reference_depth(kk)  &
                               - DIABATIC_DEPTH(i,j)
            K_LEVEL(i,j)   = k
            ZTW(i,j)       = kk
          endif
        ...
        if ( COMPUTE_TLT  .and.  K_START == k ) then
          K_START = K_START + 1
        endif
      ...
      do k = 1, km
        if ( K_LEVEL(i,j) == k  .and.  &
             ZTW(i,j) == 1 ) then
          INTERIOR_DEPTH(i,j) = zt(k)
        endif
        if ( K_LEVEL(i,j) == k  .and.  &
             ZTW(i,j) == 2 ) then
          INTERIOR_DEPTH(i,j) = zw(k)
        endif
      enddo
   ...
   !$acc end  parallel loop 
     ...
     end subroutine my_transition_layer
